...
try:
    ...
    if os.path.exists(so_path):
        # Training-task containers can hang while re-entering ninja even when
        # the extension is already built, so load the cached module directly.
    ...
    # if failed, try with JIT compilation
        ...
        if os.path.exists(os.path.join(build_dir, "gsplat_cuda.lib")):
            # If the Windows build exists, fall back to the standard loader.
            _C = load(
                name=name,
                sources=sources,
