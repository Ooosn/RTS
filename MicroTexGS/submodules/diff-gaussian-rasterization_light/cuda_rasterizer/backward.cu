	...
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			contributor--;
			const bool current_has_out = true;
			...
			// Forward only applies thres/radii_comp when a Gaussian samples
			// its own shadow value. A blocker still affects downstream T via
			// alpha inside the tile, so do not footprint-gate the blocker here.
			...
			while (offset_index >= range_start && 
				...
				// Receiver out_trans/non_trans uses exp(power) without the
				// blocker alpha 1/255 pruning threshold, so keep it exact here.
				const float G_offset = exp(power_offset);
				...
				const float effective_offset_radius = thres == -1.0f ? radii_comp[coll_id_offset] : thres;
				if (dd_offset < effective_offset_radius)
				{
			...
			if (current_has_out)
			{
				// Exact downstream accumulation. Dividing by the number of
				// downstream contributors is a stabilizer, but it is not the
				// finite-difference derivative.
				const float accd_Grad_mean = accd_Grad;
				const float inv_denom = 1.f / fmaxf(1.f - alpha, 1e-6f);
				self_grad_term += collected_dL_dout_trans[j] * T_self;
				other_grad_term_opacity = - accd_Grad_mean * inv_denom;
				const float dalpha_dG = alpha_valid ? con_o_j.w : 0.0f; // gating
				other_grad_term = - accd_Grad_mean * (dalpha_dG * inv_denom);
				CHECK_NAN(accd_Grad);
				CHECK_NAN(T_self);
			}
			// 合并得到 dL/dG_i
			// Use only downstream occlusion for footprint geometry. The self
			// normalization term is a receiver sampling effect and gives
			// wrong-sign receiver mean gradients in finite-difference checks.
			const float dL_dG_total = other_grad_term;
