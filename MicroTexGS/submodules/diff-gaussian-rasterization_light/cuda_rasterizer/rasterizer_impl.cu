...
__global__ void tileLengths(const uint2* ranges, uint32_t* toDo, int num_tiles){
    int t = blockIdx.x * blockDim.x + threadIdx.x;
    if (t >= num_tiles) return;
    uint2 r = ranges[t];
    toDo[t] = (r.x == 0xFFFFFFFFu) ? 0u : (r.y - r.x);
}
...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char*& chunk, size_t N)
{
	...
	obtain(chunk, img.id_contrib, N, 128);
	obtain(chunk, img.n_contrib_offset, N, 128);
	obtain(chunk, img.id_contrib_offset, N, 128);
	return img;
}

// CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk4backward(char*& chunk, int num_tiles)
// {
// 	ImageState img;
// 	obtain(chunk, img.current_index, num_tiles, 128);
// 	obtain(chunk, img.offset_index, num_tiles + 1, 128);
// 	return img;
// }
	...
	)
{
	...
	// cudaEvent_t e_start, e_mid, e_end, e_mmid;
	// cudaEventCreate(&e_start);
	// cudaEventCreate(&e_mid);
	// cudaEventCreate(&e_end);
	// cudaEventCreate(&e_mmid);
	// cudaEventRecord(e_start, MY_STREAM);
	...
	// cudaEventRecord(e_mid, MY_STREAM);
	...
	*/ 
	...
	// 对 imageState 进行修改
	// size_t img_chunk_size_backward = ImageState_required4backward(num_rendered);
	// // 分配/调整 imageBuffer 内存
	// char* img_chunkptr_backward = imageBuffer(img_chunk_size_backward);
	// // 根据缓冲区内存位置 初始化图片状态
	// ImageState imgState_backward = ImageState::fromChunk4backward(img_chunkptr_backward, num_rendered);
	// imgState_backward.accum_alpha = imgState.accum_alpha;     // 不拷贝
	// imgState_backward.n_contrib   = imgState.n_contrib;
	// imgState_backward.ranges      = imgState.ranges;
	// imgState = imgState_backward;

	// 如果使用了 precomputed colors，则使用 colors_precomp，否则使用 geomState.rgb
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	

	// cudaEventRecord(e_mmid, MY_STREAM);
	...
	CHECK_CUDA(FORWARD::render(
		...
		// for backward
		imgState.id_contrib,
		imgState.n_contrib_offset,
		imgState.id_contrib_offset
		), debug)

	// cudaEventRecord(e_end, MY_STREAM);	
	// 回归 host，同步流，其实可忽略，因为 return 会触发隐式同步
	// cudaStreamSynchronize(MY_STREAM);
	// float t1, t2, t3;
	// cudaEventElapsedTime(&t1, e_start, e_mid);
	// cudaEventElapsedTime(&t2, e_mid, e_mmid);
	// cudaEventElapsedTime(&t3, e_mmid, e_end);
	// std::cout << "kernel3: " << t3 << " ms\n";
	// std::cout << "kernel2: " << t2 << " ms\n";
	// std::cout << "kernel1: " << t1 << " ms\n";
	return num_rendered;
}
	...
	const float thres)
{
	...
	// 如果使用了 precomputed colors，则使用 colors_precomp，否则使用 geomState.rgb
	// geomState.rgb 是 forward 中的计算得到的 rgb
