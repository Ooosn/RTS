	...
	const float zfar)
{
  ...
  torch::Tensor final_T = torch::full({H, W}, 0.0, float_opts).contiguous();
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		debug,
		shadow.contiguous().data<float>(),
		final_T.contiguous().data<float>(),
		znear,
		zfar);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, final_T);
}
