	...
	const float zfar)
{
	...
	float3 p_view;
	if (!in_frustum(idx, orig_points, viewmatrix, projmatrix, prefiltered, p_view, znear, zfar))
		return;
	...
	float* __restrict__ final_T)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{	
			...
			if (test_T < 0.0001f && !continue_flag)
			{
				continue_flag = true;
				continue;
			}
	...
	if (inside)
	{
		...
		if (invdepth)
			invdepth[pix_id] = expected_depth/final_alpha;// 1. / (expected_depth + T * 1e3);
