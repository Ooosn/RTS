...
renderCUDA(
	...
	float* __restrict__ dL_dtex_color,
	float* __restrict__ dL_dtex_alpha)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float opa = nor_o.w;
			float tex_rgb[C] = {0};
			float tex_rgb_du[C] = {0};
			float tex_rgb_dv[C] = {0};
			float tex_a = 1.0f;
			float tex_a_du = 0.0f;
			float tex_a_dv = 0.0f;
			float u = 0.5f;
			...
			float dL_dtex_rgb[C] = {0};
			for (int ch = 0; ch < C; ch++)
			{
				const float c = collected_colors[ch * BLOCK_SIZE + j] + tex_rgb[ch];
				...
				dL_dtex_rgb[ch] = dchannel_dcolor * dL_dchannel;
			}
			...
			for (int ch = 0; ch < C; ++ch) {
				dL_ds_texture_x += dL_dtex_rgb[ch] * tex_rgb_du[ch] * du_dsx;
				dL_ds_texture_y += dL_dtex_rgb[ch] * tex_rgb_dv[ch] * dv_dsy;
			}
			dL_ds_texture_x += dL_dsample_alpha * tex_a_du * du_dsx;
			dL_ds_texture_y += dL_dsample_alpha * tex_a_dv * dv_dsy;
#if RENDER_AXUTILITY
			dL_dz += alpha * T * dL_ddepth; 
#endif

			const float2 dL_ds = {
				dL_ds_texture_x + dL_dz * Tw.x + (rho3d <= rho2d ? dL_dG * -G * s.x : 0.0f),
				dL_ds_texture_y + dL_dz * Tw.y + (rho3d <= rho2d ? dL_dG * -G * s.y : 0.0f)
			};
