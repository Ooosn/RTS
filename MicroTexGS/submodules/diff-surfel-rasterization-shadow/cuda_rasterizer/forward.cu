			...
			if (p_j.z != 0.0f)
			{
				...
				bool inside_thres = (thres <= 0.0f) || ((d_j.x * d_j.x + d_j.y * d_j.y) <= thres * thres);
					...
					if (inside_thres && power_j <= 0.0f)
					{
						...
						float du_dsx_j = 0.0f;
						float dv_dsy_j = 0.0f;
						if ((texture_shadow_use_alpha || texture_shadow_output_uv) && (texture_resolution > 0 || texture_dims != nullptr))
						{
							compute_texture_uv(s_j, texture_sigma_factor, u_j, v_j, du_dsx_j, dv_dsy_j);
						}
						if (texture_shadow_use_alpha && texture_alpha != nullptr && (texture_resolution > 0 || texture_dims != nullptr))
						{
							if (texture_shadow_alpha_bilinear)
							{
							...
							{
								tex_a_j = sample_texture_alpha_nearest(
									texture_alpha,
									current_id,
									texture_resolution,
									texture_dims,
					...
					if (p_o.z != 0.0f)
					{
						...
						float rho2d_o = FilterInvSquare * (d_o.x * d_o.x + d_o.y * d_o.y);
							...
							if (power_o <= 0.0f)
							{
								...
								{
									tex_a_o = sample_texture_alpha_nearest(
										texture_alpha,
										offset_id,
										texture_resolution,
										texture_dims,
				...
				if (depth_diff <= offset)
				{
					if (!done && current_valid)
					{
						...
						float transition_trans = footprint_j * (T * (1.0f - (depth_diff / offset) + (depth_diff / offset) * temp));
							if (texture_shadow_output_uv && (texture_resolution > 0 || texture_dims != nullptr))
							{
								int tex_h = 0;
								int tex_w = 0;
								int tex_offset = 0;
...
void FORWARD::renderColor(
	...
	float* out_color)
{
	renderColorCUDA<NUM_CHANNELS> << <grid, block, 0, MY_STREAM >> > (
		ranges,
		point_list,
		W, H,
		means2D,
