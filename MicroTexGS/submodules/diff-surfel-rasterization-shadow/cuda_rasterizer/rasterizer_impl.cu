	...
	bool debug)
{
	...
	CHECK_CUDA(cudaStreamSynchronize(MY_STREAM), debug);
	...
	CHECK_CUDA(FORWARD::render(
		...
		non_trans,
			...
			texture_shadow_alpha_bilinear), debug)

	return num_rendered;
}

int CudaRasterizer::Rasterizer::forwardColor(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Preprocess: project Gaussians, fill geomState
	CHECK_CUDA(FORWARD::preprocess(
		P, D, M,
		means3D,
		(glm::vec2*)scales,
	...
	CHECK_CUDA(cub::DeviceScan::InclusiveSum(geomState.scanning_space, geomState.scan_size,
		geomState.tiles_touched, geomState.point_offsets, P, MY_STREAM), debug)
