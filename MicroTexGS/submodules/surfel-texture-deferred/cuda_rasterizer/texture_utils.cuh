...
{
    return (((gid * TEXTURE_CHANNELS + ch) * tex_res + y) * tex_res + x);
}
...
__device__ inline int texture_color_index(int gid, int ch, int y, int x, int tex_res, const int* texture_dims)
{
    if (texture_dims != nullptr)
    {
        ...
        return ((tex_offset + y * tex_w + x) * TEXTURE_CHANNELS + ch);
    }
    ...
    float& d_alpha_dv)
{
    ...
    if (tex_h == 1 && tex_w == 1)
    {
        ...
        {
            for (int ch = 0; ch < TEXTURE_CHANNELS; ++ch)
            {
...
__device__ inline void sample_texture_color_bilinear_forward(
    const float* texture_color,
    const int gid,
    const int tex_res,
    const int* texture_dims,
...
__device__ inline float sample_texture_alpha_bilinear_forward(
    const float* texture_alpha,
    const int gid,
    const int tex_res,
    const int* texture_dims,
    ...
    const float v)
{
    ...
    if (tex_h == 1 && tex_w == 1)
    {
        return texture_alpha[texture_alpha_index(gid, 0, 0, tex_res, texture_dims)];
    }
