	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float opa = nor_o.w;
			float tex_rgb[CHANNELS] = {0};
			float tex_a = 1.0f;
			float u = 0.5f;
			float v = 0.5f;
			float du_dsx = 0.0f;
			float dv_dsy = 0.0f;
			const bool has_texture = use_textures && texture_color != nullptr && (texture_resolution > 0 || texture_dims != nullptr);
			const bool has_texture_alpha = has_texture && texture_alpha != nullptr;

			if (has_texture_alpha)
			{
				compute_texture_uv(s, texture_sigma_factor, u, v, du_dsx, dv_dsy);
				tex_a = sample_texture_alpha_bilinear_forward(
					texture_alpha,
					collected_id[j],
					texture_resolution,
					texture_dims,
			...
			if (has_texture)
			{
				if (!has_texture_alpha)
					compute_texture_uv(s, texture_sigma_factor, u, v, du_dsx, dv_dsy);
				sample_texture_color_bilinear_forward(
					texture_color,
					collected_id[j],
					texture_resolution,
					texture_dims,
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += (features[collected_id[j] * CHANNELS + ch] + tex_rgb[ch]) * w;
			T = test_T;
