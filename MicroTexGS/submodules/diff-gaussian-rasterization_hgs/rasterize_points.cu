	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_dopacity1.contiguous().data<float>(),
	  dL_dopacity2.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, torch::cat({dL_dopacity1, dL_dopacity2}, /*dim=*/1), dL_dmeans3D, dL_dnormal, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
