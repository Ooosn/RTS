...
def _normalize_compare_path(path_value):
    return os.path.realpath(os.path.abspath(os.path.expanduser(str(path_value))))


def _checkpoint_cfg_source_path(checkpoint):
    cfg_path = Path(checkpoint).resolve().parent / "cfg_args"
    if not cfg_path.exists():
        return None
    text = cfg_path.read_text(encoding="utf-8", errors="replace")
    match = re.search(r"\bsource_path='([^']*)'", text)
    if match is None:
        return None
    return _normalize_compare_path(match.group(1))


def _assert_checkpoint_source_matches(checkpoint, requested_source_path):
    cfg_source = _checkpoint_cfg_source_path(checkpoint)
    if cfg_source is None:
        return
    requested_source = _normalize_compare_path(requested_source_path)
    if cfg_source != requested_source:
        raise RuntimeError(
            "Checkpoint/source_path mismatch:\n"
            f"  checkpoint: {checkpoint}\n"
            f"  checkpoint cfg source_path: {cfg_source}\n"
            f"  requested source_path:       {requested_source}\n"
            "Refusing to resume with a checkpoint from a different dataset."
        )


def _run_command_for_manifest(cmd, cwd):
    try:
        ...
        return Namespace(returncode=-1, stdout="", stderr=repr(exc))
...
def write_run_manifest(args):
    ...
    manifest = {
        ...
        "checkpoint_first_iter": getattr(args, "checkpoint_first_iter", None),
        "iterations": getattr(args, "iterations", None),
        "test_iterations": list(getattr(args, "test_iterations", []) or []),
        "save_iterations": list(getattr(args, "save_iterations", []) or []),
        "checkpoint_iterations": list(getattr(args, "checkpoint_iterations", []) or []),
        "eval_only": bool(getattr(args, "eval_only", False)),
        "use_textures": bool(getattr(args, "use_textures", False)),
        "texture_start_iter": getattr(args, "texture_start_iter", None),
        "texture_effect_mode": getattr(args, "texture_effect_mode", None),
        "texture_dynamic_resolution": bool(getattr(args, "texture_dynamic_resolution", False)),
        "texture_shadow_confidence_enabled": bool(getattr(args, "texture_shadow_confidence_enabled", False)),
        "texture_shadow_confidence_start_iter": getattr(args, "texture_shadow_confidence_start_iter", None),
        "texture_shadow_confidence_strength": getattr(args, "texture_shadow_confidence_strength", None),
        "texture_shadow_confidence_gamma": getattr(args, "texture_shadow_confidence_gamma", None),
        "texture_shadow_confidence_zero_dc": bool(getattr(args, "texture_shadow_confidence_zero_dc", False)),
        "texture_shadow_transport_mode": getattr(args, "texture_shadow_transport_mode", None),
        "texture_shadow_transport_eps": getattr(args, "texture_shadow_transport_eps", None),
        "texture_factor_surgery": getattr(args, "texture_factor_surgery", None),
        "texture_factor_surgery_seed": getattr(args, "texture_factor_surgery_seed", None),
        "texture_rtg_enabled": bool(getattr(args, "texture_rtg_enabled", False)),
        "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
        "pythonpath": os.environ.get("PYTHONPATH"),
    }

    root_proc = _run_command_for_manifest(["git", "rev-parse", "--show-toplevel"], cwd)
    git_root = root_proc.stdout.strip() if root_proc.returncode == 0 else None
    manifest["git_root"] = git_root
    if git_root:
        head_proc = _run_command_for_manifest(["git", "rev-parse", "HEAD"], git_root)
        branch_proc = _run_command_for_manifest(["git", "branch", "--show-current"], git_root)
        status_proc = _run_command_for_manifest(["git", "status", "--short"], git_root)
        diff_proc = _run_command_for_manifest(["git", "diff", "--"], git_root)
        manifest.update(
            {
                "git_head": head_proc.stdout.strip() if head_proc.returncode == 0 else None,
                "git_branch": branch_proc.stdout.strip() if branch_proc.returncode == 0 else None,
                "git_dirty": bool(status_proc.stdout.strip()) if status_proc.returncode == 0 else None,
            }
        )
        (model_path / "run_git_status.txt").write_text(
            status_proc.stdout + status_proc.stderr,
            encoding="utf-8",
            errors="replace",
        )
        (model_path / "run_git_diff.patch").write_text(
            diff_proc.stdout + diff_proc.stderr,
            encoding="utf-8",
            errors="replace",
        )
    else:
        manifest["git_error"] = root_proc.stderr.strip()
    ...
    snapshot_files = [
        ...
        "scene/__init__.py",
        "scene/gaussian_model_2dgs_adapter.py",
        "scene/gaussian_model_native_2dgs.py",
        "scene/neural_phase_function.py",
    ]
    snapshot_dir = model_path / "source_snapshot"
    ...
    manifest["source_snapshot_files"] = copied_files

    (model_path / "run_manifest.json").write_text(
...
def log_texture_architecture(path: Path, iteration: int, event: str, gaussians, modelset=None, pipe=None, opt=None, print_summary=True):
    ...
    if modelset is not None:
        payload["model_flags"] = {
            ...
            "texture_shadow_confidence_enabled": bool(getattr(modelset, "texture_shadow_confidence_enabled", False)),
            "texture_shadow_confidence_start_iter": getattr(modelset, "texture_shadow_confidence_start_iter", None),
            "texture_shadow_confidence_strength": getattr(modelset, "texture_shadow_confidence_strength", None),
            "texture_shadow_confidence_gamma": getattr(modelset, "texture_shadow_confidence_gamma", None),
            "texture_shadow_confidence_zero_dc": bool(getattr(modelset, "texture_shadow_confidence_zero_dc", False)),
            "texture_shadow_transport_mode": getattr(modelset, "texture_shadow_transport_mode", None),
            "texture_shadow_transport_eps": getattr(modelset, "texture_shadow_transport_eps", None),
            "texture_factor_surgery": getattr(modelset, "texture_factor_surgery", None),
            "texture_factor_surgery_seed": getattr(modelset, "texture_factor_surgery_seed", None),
        }
    if pipe is not None:
...
def _parameter_health(parameter):
    ...
    grad = parameter.grad
    return {
        "present": bool(parameter.numel() > 0),
        "requires_grad": bool(getattr(parameter, "requires_grad", False)),
        "finite": _tensor_finite(parameter),
        "absmax": _tensor_absmax(parameter),
        "grad_present": grad is not None,
        "grad_finite": _tensor_finite(grad) if grad is not None else True,
        "grad_absmax": _tensor_absmax(grad),
    }
...
def compute_texture_oob_regularization(direct_image, other_effects, opt, iteration):
    """Limit Gaussian-level other_effects where MicroTexGS direct transport is observable."""
    if not bool(getattr(opt, "texture_oob_enabled", False)):
        return None
    weight = float(getattr(opt, "texture_oob_weight", 0.0))
    if weight <= 0.0:
        return None
    if int(iteration) < int(getattr(opt, "texture_oob_start_iter", 30_000)):
        return None
    if direct_image is None or other_effects is None:
        return None

    eps = 1e-8
    direct_abs = torch.clamp(direct_image.detach(), 0.0, 1.0).abs().mean(dim=0, keepdim=True)
    other_abs = other_effects.abs().mean(dim=0, keepdim=True)
    scale = max(float(getattr(opt, "texture_oob_gate_scale", 1.0)), eps)
    power = max(float(getattr(opt, "texture_oob_gate_power", 1.0)), eps)
    target_ratio = max(float(getattr(opt, "texture_oob_target_ratio", 0.0)), 0.0)
    gate = torch.clamp(direct_abs / (direct_abs.mean().clamp_min(eps) * scale), 0.0, 1.0)
    if abs(power - 1.0) > 1e-8:
        gate = gate.pow(power)
    over_budget = torch.relu(other_abs - target_ratio * direct_abs)
    raw = (gate * (over_budget / direct_abs.mean().clamp_min(eps))).sum() / gate.sum().clamp_min(1.0)
    return {
        "total": raw * weight,
        "raw": raw.detach(),
        "gate_mean": gate.detach().mean(),
        "target_ratio": torch.tensor(target_ratio, dtype=raw.dtype, device=raw.device),
        "other_abs_mean": other_abs.detach().mean(),
        "direct_abs_mean": direct_abs.detach().mean(),
    }


def texture_shadow_confidence_mode_supported(gaussians):
    return str(getattr(gaussians, "texture_effect_mode", "")) in {
...
def collect_training_health(iteration, ll1, loss, image, shadow, other_effects, gaussians, num_points, densify_due, opacity_reset_due, shadow_stage, modelset, texture_oob_reg=None, texture_rmd_reg=None):
    ...
    texture_health = {
        "tex_color": _parameter_health(getattr(gaussians, "_tex_color", None)),
        "tex_alpha": _parameter_health(getattr(gaussians, "_tex_alpha", None)),
        "tex_specular": _parameter_health(getattr(gaussians, "_tex_specular", None)),
        "tex_normal": _parameter_health(getattr(gaussians, "_tex_normal", None)),
    }
    texture_finite = all(item["finite"] and item["grad_finite"] for item in texture_health.values())
    ...
    health = {
        ...
        "texture_effect_mode": getattr(gaussians, "texture_effect_mode", None),
        "texture_dynamic_resolution": bool(getattr(gaussians, "texture_dynamic_resolution", False)),
        "texture_has_dynamic": bool(getattr(gaussians, "has_dynamic_textures", False)),
        "texture_tor_enabled": bool(getattr(gaussians, "texture_tor_enabled", False)),
        "texture_tor_active": bool(
            getattr(gaussians, "texture_tor_enabled", False)
            and int(iteration) >= int(getattr(gaussians, "texture_tor_start_iter", 30_000))
        ),
        "texture_tor_start_iter": int(getattr(gaussians, "texture_tor_start_iter", 30_000)),
        "texture_tor_gate_floor": float(getattr(gaussians, "texture_tor_gate_floor", 0.05)),
        "texture_tor_gate_scale": float(getattr(gaussians, "texture_tor_gate_scale", 1.0)),
        "texture_tor_strength": float(getattr(gaussians, "texture_tor_strength", 1.0)),
        "texture_shadow_confidence_enabled": bool(getattr(gaussians, "texture_shadow_confidence_enabled", False)),
        "texture_shadow_confidence_mode_supported": bool(texture_shadow_confidence_mode_supported(gaussians)),
        "texture_shadow_confidence_active": bool(
            getattr(gaussians, "texture_shadow_confidence_enabled", False)
            and texture_shadow_confidence_mode_supported(gaussians)
            and int(iteration) >= int(getattr(gaussians, "texture_shadow_confidence_start_iter", 30_000))
        ),
        "texture_shadow_confidence_start_iter": int(getattr(gaussians, "texture_shadow_confidence_start_iter", 30_000)),
        "texture_shadow_confidence_strength": float(getattr(gaussians, "texture_shadow_confidence_strength", 1.0)),
        "texture_shadow_confidence_gamma": float(getattr(gaussians, "texture_shadow_confidence_gamma", 1.0)),
        "texture_shadow_confidence_zero_dc": bool(getattr(gaussians, "texture_shadow_confidence_zero_dc", False)),
        "texture_shadow_transport_mode": str(getattr(gaussians, "texture_shadow_transport_mode", "additive")),
        "texture_shadow_transport_eps": float(getattr(gaussians, "texture_shadow_transport_eps", 1e-4)),
        "texture_factor_surgery": str(getattr(gaussians, "texture_factor_surgery", "none")),
        "texture_factor_surgery_seed": int(getattr(gaussians, "texture_factor_surgery_seed", 0)),
        "texture_oob_enabled": bool(texture_oob_reg is not None),
        "texture_oob_total": None if texture_oob_reg is None else float(texture_oob_reg["total"].detach().item()),
        "texture_oob_raw": None if texture_oob_reg is None else float(texture_oob_reg["raw"].detach().item()),
        "texture_oob_gate_mean": None if texture_oob_reg is None else float(texture_oob_reg["gate_mean"].detach().item()),
    ...
    for name, item in texture_health.items():
        for key, value in item.items():
            health[f"{name}_{key}"] = value
    return health
...
def save_fixed_view_artifacts(
    ...
    shadow: torch.Tensor,
    metrics: dict,
    other_effects: Optional[torch.Tensor] = None,
):
    ...
    other_img = Image.fromarray(_tensor_to_uint8_hwc(other_effects)) if other_effects is not None else None
    ...
    if other_img is not None:
        other_img.save(output_dir / "other.png")
    ...
    panel = Image.new("RGB", (w * len(images), h + title_h + footer_h), (255, 255, 255))
...
def training(modelset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, unfreeze_iterations, debug_from, eval_only=False):
    first_iter = 0
    checkpoint_model_params = None
    checkpoint_scene_state = None
    checkpoint_first_iter = None
    if checkpoint:
        checkpoint_payload = torch.load(checkpoint, weights_only=False)
        if not isinstance(checkpoint_payload, (tuple, list)) or len(checkpoint_payload) < 2:
            raise ValueError(f"Unsupported checkpoint payload in {checkpoint}")
        checkpoint_model_params = checkpoint_payload[0]
        checkpoint_first_iter = checkpoint_payload[1]
        if len(checkpoint_payload) >= 3:
            checkpoint_scene_state = checkpoint_payload[2]
        _assert_checkpoint_source_matches(checkpoint, modelset.source_path)
        print(f"Loaded checkpoint: {checkpoint} (iteration {checkpoint_first_iter})")

    # 准备输出和日志记录器，更新模型路径
    # prepare_output_and_logger receives ModelParams, not the raw argparse
    # Namespace. Attach the training context explicitly so cfg_args and
    # run_manifest can audit resume experiments instead of looking like from0.
    modelset.start_checkpoint = checkpoint
    modelset.checkpoint_first_iter = checkpoint_first_iter
    modelset.iterations = opt.iterations
    modelset.test_iterations = list(testing_iterations or [])
    modelset.save_iterations = list(saving_iterations or [])
    modelset.checkpoint_iterations = list(checkpoint_iterations or [])
    modelset.eval_only = bool(eval_only)
    modelset.texture_start_iter = int(getattr(opt, "texture_start_iter", 0))
    modelset.texture_rtg_enabled = bool(getattr(opt, "texture_rtg_enabled", False))
    ...
    scene = Scene(
        ...
        load_optimized_cameras=checkpoint_scene_state is None,
    )
    ...
    if checkpoint:
        ...
        if (
            str(getattr(modelset, "rasterizer", "")).startswith("2dgs")
            and getattr(gaussians, "gs2dgs_backend", "") == "native"
        ):
            gaussians.restore(checkpoint_model_params, opt, load_optimizer=False)
            gaussians.training_setup(opt)
        else:
            gaussians.restore(checkpoint_model_params, opt)
        if checkpoint_scene_state is not None:
            scene.restore(checkpoint_scene_state)
    ...
    if bool(getattr(opt, "texture_rtg_enabled", False)) and bool(getattr(opt, "texture_rtg_freeze_gaussian_densify", True)):
        gaussian_densify_until_iter = min(
            ...
            int(getattr(opt, "texture_rtg_refine_from_iter", gaussian_densify_until_iter)),
        )
        print(f"Gaussian densification capped at iteration {gaussian_densify_until_iter} before RTG texture refinement")
    has_restored_textures = bool(
        getattr(gaussians, "use_textures", False)
        and hasattr(gaussians, "_tex_color")
        and torch.is_tensor(getattr(gaussians, "_tex_color"))
        and getattr(gaussians, "_tex_color").numel() > 0
    )
    if texture_requested and texture_start_iter > 0 and first_iter <= texture_start_iter and not has_restored_textures:
        if hasattr(gaussians, "defer_texture_training"):
            gaussians.defer_texture_training()
            print(f"Texture training deferred until after iteration {texture_start_iter}")
    elif has_restored_textures and texture_start_iter > 0 and first_iter <= texture_start_iter:
        print(f"Texture checkpoint state restored at iteration {first_iter}; not deferring texture training.")
    ...
    if eval_only:
        eval_iteration = int(checkpoint_first_iter if checkpoint_first_iter is not None else max(first_iter - 1, 0))
        eval_render_args = {
            "modelset": modelset,
            "pipe": pipe,
            "bg_color": background,
        ...
        if eval_iteration < opt.spcular_freeze_step + opt.fit_linear_step:
            eval_render_args["fix_labert"] = True
        log_texture_architecture(
            texture_arch_log_path,
            eval_iteration,
            "eval_only",
        ...
        zero = torch.zeros((), dtype=torch.float32, device=gaussians.get_xyz.device)
        with torch.no_grad():
            training_report(
                tb_writer,
                eval_iteration,
                ...
                render,
                eval_render_args,
                gamma=2.2 if modelset.hdr else 1.0,
    ...
    while iteration <= opt.iterations:    # Main iteration counts train-view updates only; test-view pose/light steps do not advance it.
        ...
        if iteration <= unfreeze_iterations:    # 5000
            ...
            direct_image = image * shadow
            image = direct_image + other_effects
        ...
        # 尚未更新参数，等待后续挑选更新
        loss.backward()
        if not (opt_test and scene.optimizing) and hasattr(gaussians, "accumulate_texture_rtg"):
        ...
        with torch.no_grad():
            densify_due = iteration < gaussian_densify_until_iter and iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0
            ...
            # 1) 对于非测试集，即训练集进入正常阶段，先进行高斯密集化，高斯修剪以及场景参数优化:
                ...
                if iteration < gaussian_densify_until_iter:
                    ...
                    if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                        ...
                        num_points_before = gaussians.get_xyz.shape[0]
                        clone_grad_threshold = float(getattr(opt, "densify_clone_grad_threshold", 0.0) or opt.densify_grad_threshold)
                        if (
                            bool(getattr(modelset, "use_textures", False))
                            and iteration >= texture_start_iter
                        ):
                            clone_grad_threshold *= float(getattr(opt, "texture_clone_threshold_scale", 1.0))
                        num_clone, num_split = gaussians.densify_and_prune(
                            opt.densify_grad_threshold,
                            0.005,
                            scene.cameras_extent,
                            size_threshold,
                            opt.bigsize_threshold,
                            crop_extent,
                        ...
                        densify_diag.update(
                            {
                                "split_grad_threshold": float(opt.densify_grad_threshold),
                                "clone_grad_threshold": float(clone_grad_threshold),
                                "texture_clone_threshold_scale": float(getattr(opt, "texture_clone_threshold_scale", 1.0)),
                ...
                if iteration < opt.iterations:  # 判断是否处于优化阶段
                    ...
                    rtg_due = False
                    rtg_due_target = None
                    if hasattr(gaussians, "texture_rtg_enabled"):
                        rtg_from = int(getattr(gaussians, "texture_rtg_refine_from_iter", 0))
                        rtg_until = int(getattr(gaussians, "texture_rtg_refine_until_iter", 0))
                        rtg_interval = max(1, int(getattr(gaussians, "texture_rtg_refine_interval", 1)))
                        if hasattr(gaussians, "texture_rtg_due_target"):
                            rtg_due_target = gaussians.texture_rtg_due_target(iteration)
                            rtg_due = rtg_due_target is not None
                        else:
                            rtg_due = (
                                bool(getattr(gaussians, "texture_rtg_enabled", False))
                                and iteration > rtg_from
                                and iteration <= rtg_until
                                and (iteration - rtg_from) % rtg_interval == 0
                            )
                            rtg_due_target = iteration if rtg_due else None
                        if bool(getattr(gaussians, "texture_rtg_enabled", False)) and iteration % 1000 == 0:
                            denom = getattr(gaussians, "_rtg_denom", None)
                            denom_nonzero = int((denom > 0).sum().item()) if isinstance(denom, torch.Tensor) else -1
                            denom_mean = float(denom.float().mean().item()) if isinstance(denom, torch.Tensor) and denom.numel() > 0 else 0.0
                            print(
                                f"\n[ITER {iteration}] RTG probe | "
                                f"due={rtg_due} target={rtg_due_target} enabled={getattr(gaussians, 'texture_rtg_enabled', None)} "
                                f"from={getattr(gaussians, 'texture_rtg_refine_from_iter', None)} "
                                f"until={getattr(gaussians, 'texture_rtg_refine_until_iter', None)} "
                                f"interval={getattr(gaussians, 'texture_rtg_refine_interval', None)} "
                                f"denom_nonzero={denom_nonzero} denom_mean={denom_mean:.1f}"
                            )
                    if hasattr(gaussians, "update_texture_rtg_probe_window"):
                        rtg_probe_log = gaussians.update_texture_rtg_probe_window(iteration)
                        if rtg_probe_log:
                            print(f"\n[ITER {iteration}] RTG probe-window | {rtg_probe_log}")
                    if hasattr(gaussians, "refine_textures_by_rtg"):
                        rtg_refined = gaussians.refine_textures_by_rtg(iteration)
                        rtg_log = gaussians.texture_rtg_log_string() if hasattr(gaussians, "texture_rtg_log_string") else ""
                        if rtg_log:
                            if rtg_refined > 0:
                                print(f"\n[ITER {iteration}] RTG refined {rtg_refined} texture charts | {rtg_log}")
                            else:
                            ...
                            denom_mean = float(denom.float().mean().item()) if isinstance(denom, torch.Tensor) and denom.numel() > 0 else 0.0
                            print(
                                f"\n[ITER {iteration}] RTG due but produced no log | "
                                f"enabled={getattr(gaussians, 'texture_rtg_enabled', None)} "
                                f"dynamic={getattr(gaussians, 'texture_dynamic_resolution', None)} "
                                f"has_dynamic={getattr(gaussians, 'has_dynamic_textures', None)} "
                                f"from={getattr(gaussians, 'texture_rtg_refine_from_iter', None)} "
                                f"until={getattr(gaussians, 'texture_rtg_refine_until_iter', None)} "
                                f"interval={getattr(gaussians, 'texture_rtg_refine_interval', None)} "
                                f"denom_nonzero={denom_nonzero} denom_mean={denom_mean:.1f}"
                ...
                eval_row.update(
                    {
                        ...
                        "shadow_backward_scaling_enabled": bool(shadow_stage["scaling"]),
                        "shadow_backward_rotation_enabled": bool(shadow_stage["rotation"]),
                        "texture_shadow_confidence_enabled": bool(getattr(scene.gaussians, "texture_shadow_confidence_enabled", False)),
                        "texture_shadow_confidence_mode_supported": bool(texture_shadow_confidence_mode_supported(scene.gaussians)),
                        "texture_shadow_confidence_active": bool(
                            getattr(scene.gaussians, "texture_shadow_confidence_enabled", False)
                            and texture_shadow_confidence_mode_supported(scene.gaussians)
                            and int(iteration) >= int(getattr(scene.gaussians, "texture_shadow_confidence_start_iter", 30_000))
                        ),
                        "texture_shadow_confidence_start_iter": int(getattr(scene.gaussians, "texture_shadow_confidence_start_iter", 30_000)),
                        "texture_shadow_confidence_strength": float(getattr(scene.gaussians, "texture_shadow_confidence_strength", 1.0)),
                        "texture_shadow_confidence_gamma": float(getattr(scene.gaussians, "texture_shadow_confidence_gamma", 1.0)),
                        "texture_shadow_confidence_zero_dc": bool(getattr(scene.gaussians, "texture_shadow_confidence_zero_dc", False)),
                        "texture_shadow_transport_mode": str(getattr(scene.gaussians, "texture_shadow_transport_mode", "additive")),
                        "texture_shadow_transport_eps": float(getattr(scene.gaussians, "texture_shadow_transport_eps", 1e-4)),
                        "texture_factor_surgery": str(getattr(scene.gaussians, "texture_factor_surgery", "none")),
                        "texture_factor_surgery_seed": int(getattr(scene.gaussians, "texture_factor_surgery_seed", 0)),
                    }
                )
...
if __name__ == "__main__":
    ...
    if args.use_textures and args.texture_start_iter > 0:
        args.checkpoint_iterations.append(args.texture_start_iter)
        args.save_iterations.append(args.texture_start_iter)
        args.test_iterations.append(args.texture_start_iter)
    ...
    # Start training
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.unfreeze_iterations, args.debug_from, eval_only=args.eval_only)
