...
_SUBMODULE_PATHS = [
    ...
    os.path.join(_REPO_ROOT, "..", "gs3", "submodules", "diff-gaussian-rasterization"),
    os.path.join(_REPO_ROOT, "..", "gs3", "submodules", "diff-gaussian-rasterization_light"),
    os.path.join(_REPO_ROOT, "..", "gs3", "submodules", "diff-gaussian-rasterization_hgs"),
    os.path.join(_REPO_ROOT, "..", "gs3", "submodules", "v_3dgs"),
    os.path.join(_REPO_ROOT, "..", "gs3", "submodules", "v_3dgs_ortho"),
...
_COMPILED_SUBMODULE_PACKAGES = {
    ...
    "diff-gaussian-rasterization_light": "diff_gaussian_rasterization_light",
    "diff-gaussian-rasterization_hgs": "diff_gaussian_rasterization_hgs",
    "v_3dgs": "v_3dgs",
    "v_3dgs_ortho": "v_3dgs_ortho",
...
for _path in reversed(_SUBMODULE_PATHS):
    _package = _COMPILED_SUBMODULE_PACKAGES.get(os.path.basename(_path))
    if _package and not _has_compatible_extension(_path, _package):
        continue
...
def render(viewpoint_camera, 
    ...
    if override_color is None:
        if gau.use_MBRDF:
            ...
            shadow2, other_effects, _, __ = gau.neural_phasefunc(wi, wo, gau.get_xyz, gau.get_neural_material, \
                                                encoding_pos, None, \
                                                shadow_for_phase) # (N, 1), (N, 3)
    ...
    for var_name, var in ([
        ...
        ("decay", decay), ("other_effects", other_effects), ("asg_1", asg_1),
        ("shadow", shadow), ("rendered_image", rendered_image),
