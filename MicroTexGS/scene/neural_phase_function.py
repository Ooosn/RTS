...
class _TorchFrequencyEncoding(nn.Module):
    def __init__(self, n_input_dims, encoding_config):
        super().__init__()
        self.n_input_dims = n_input_dims
        self.n_frequencies = int(encoding_config["n_frequencies"])
...
class _TorchLikeNetwork(nn.Module):
    def __init__(self, n_input_dims, n_output_dims, network_config):
        super().__init__()
        hidden_dim = int(network_config["n_neurons"])
        hidden_layers = int(network_config["n_hidden_layers"])
...
class Neural_phase(nn.Module):
    def __init__(self, hidden_feature_size=32, hidden_feature_layers=3, frequency=4, neural_material_size=6, asg_mlp=False, mlp_zero=False):
        ...
        # ASG/specular is kept analytic. Neural_phase only predicts shadow decay
        # and other effects; legacy asg_mlp arguments are ignored for compatibility.
    ...
    def _switch_to_legacy_torch_backend(self):
        device = None
        dtype = None
        ...
        encoding_config = {
            "otype": "Frequency",
            "n_frequencies": self.frequency,
        }
        shadow_config = {
            "otype": "FullyFusedMLP",
            "activation": "LeakyReLU",
            "output_activation": "Sigmoid",
            "n_neurons": self.hidden_feature_size,
            "n_hidden_layers": self.hidden_feature_layers,
        }
        other_effects_config = {
            "otype": "FullyFusedMLP",
            "activation": "LeakyReLU",
        ...
        self.encoding = _TorchFrequencyEncoding(3, encoding_config)
        self.shadow_func = _TorchLikeNetwork(
            self.neural_material_size + self.encoding.n_output_dims * 3 + 1,
            1,
            shadow_config,
        )
        self.other_effects_func = _TorchLikeNetwork(
            self.neural_material_size + self.encoding.n_output_dims * 3,
            3,
            other_effects_config,
        )
    ...
    def load_state_dict(self, state_dict, strict=True, assign=False):
        if isinstance(state_dict, (dict, OrderedDict)) and any(
            key.startswith("shadow_func.net.") or key.startswith("other_effects_func.net.")
            for key in state_dict.keys()
        ):
            self._switch_to_legacy_torch_backend()
    ...
    def forward(self, wi, wo, pos, neural_material, encoding_pos = None, opacity = None, hint = None, asg_1 = None, asg_mlp = False):
        ...
        return output1, other_effects, None, encoding_pos
