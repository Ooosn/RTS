...
_LEGACY_2DGS_SRC = _WORKSPACE_ROOT / "2dgs" / "scene" / "gaussian_model.py"
_LOCAL_SIMPLE_KNN = _REPO_ROOT / "submodules" / "simple-knn"
_LOCAL_NATIVE_DIFF_SURFEL = _WORKSPACE_ROOT / "2dgs" / "submodules" / "diff-surfel-rasterization"
...
_LEGACY_AVAILABLE = _LEGACY_2DGS_SRC.is_file()
if _LEGACY_AVAILABLE:
    ...
    _LegacyGaussianModel2DGS = object
...
class _LegacyNoTextureAdapter(_LegacyGaussianModel2DGS):
    def __init__(self, modelset, opt=None):
        if not _LEGACY_AVAILABLE:
            raise RuntimeError(
                "Legacy 2DGS core is unavailable after workspace cleanup. "
                "Use --use_textures/--force_native_2dgs_core or restore the legacy 2dgs source."
            )
        super().__init__(
