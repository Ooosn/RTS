...
_SIMPLE_KNN_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "submodules", "simple-knn"))
if os.path.isdir(_SIMPLE_KNN_PATH) and _SIMPLE_KNN_PATH not in sys.path:
    sys.path.insert(0, _SIMPLE_KNN_PATH)
...
class GaussianModel:
    ...
    def __init__(
        ...
        self._rtg_grad_accum = torch.empty(0)
        self._rtg_denom = torch.empty(0)
        self._rtg_stability_count = torch.empty(0, dtype=torch.int16)
        ...
        self._last_rtg_refine_target_iter = 0
    ...
    def capture(self):
        return (
            ...
            None if self.neural_phasefunc is None else self.neural_phasefunc.state_dict(),
            {
                ...
                "rtg_grad_accum": self._rtg_grad_accum,
                "rtg_denom": self._rtg_denom,
                "rtg_stability_count": self._rtg_stability_count,
                ...
                "texture_rtg_resize_mode": self.texture_rtg_resize_mode,
                "texture_rtg_dry_run": self.texture_rtg_dry_run,
                "texture_rtg_stability_windows": self.texture_rtg_stability_windows,
                "texture_rtg_probe_interval": self.texture_rtg_probe_interval,
                ...
                "texture_tor_gate_scale": self.texture_tor_gate_scale,
                "texture_tor_strength": self.texture_tor_strength,
                "texture_shadow_confidence_enabled": self.texture_shadow_confidence_enabled,
                "texture_shadow_confidence_start_iter": self.texture_shadow_confidence_start_iter,
                "texture_shadow_confidence_strength": self.texture_shadow_confidence_strength,
                "texture_shadow_confidence_gamma": self.texture_shadow_confidence_gamma,
                "texture_shadow_confidence_zero_dc": self.texture_shadow_confidence_zero_dc,
                "texture_shadow_transport_mode": self.texture_shadow_transport_mode,
                "texture_shadow_transport_eps": self.texture_shadow_transport_eps,
                "texture_factor_surgery": self.texture_factor_surgery,
    ...
    def restore(self, model_args, training_args, load_optimizer=True):
        ...
        if len(model_args) == 12:
            ...
            self.texture_effect_mode = str(extra_state.get("texture_effect_mode", getattr(self, "texture_effect_mode", "uvshadow_specular_lobe")))
            ...
            self._invalidate_dynamic_texture_layout_cache()
            ...
            rtg_grad_accum = extra_state.get("rtg_grad_accum", torch.empty(0))
            self._rtg_grad_accum = rtg_grad_accum.to(device="cuda", dtype=torch.float32) if isinstance(rtg_grad_accum, torch.Tensor) else torch.empty(0, device="cuda")
            rtg_denom = extra_state.get("rtg_denom", torch.empty(0))
            self._rtg_denom = rtg_denom.to(device="cuda", dtype=torch.float32) if isinstance(rtg_denom, torch.Tensor) else torch.empty(0, device="cuda")
            rtg_stability_count = extra_state.get("rtg_stability_count", torch.empty(0))
            self._rtg_stability_count = rtg_stability_count.to(device="cuda", dtype=torch.int16) if isinstance(rtg_stability_count, torch.Tensor) else torch.empty(0, device="cuda", dtype=torch.int16)
            rtg_probe_pass_count = extra_state.get("rtg_probe_pass_count", torch.empty(0))
            self._rtg_probe_pass_count = rtg_probe_pass_count.to(device="cuda", dtype=torch.int16) if isinstance(rtg_probe_pass_count, torch.Tensor) else torch.empty(0, device="cuda", dtype=torch.int16)
            rtg_probe_window_count = extra_state.get("rtg_probe_window_count", torch.empty(0))
            ...
            self.texture_rtg_resize_mode = str(extra_state.get("texture_rtg_resize_mode", getattr(self, "texture_rtg_resize_mode", "activated_seeded_bilinear_exact"))).lower()
            self.texture_rtg_dry_run = bool(extra_state.get("texture_rtg_dry_run", getattr(self, "texture_rtg_dry_run", False)))
            self.texture_rtg_stability_windows = int(extra_state.get("texture_rtg_stability_windows", getattr(self, "texture_rtg_stability_windows", 1)))
            self.texture_rtg_probe_interval = int(extra_state.get("texture_rtg_probe_interval", getattr(self, "texture_rtg_probe_interval", 0)))
            ...
            self.texture_tor_gate_scale = float(extra_state.get("texture_tor_gate_scale", getattr(self, "texture_tor_gate_scale", 1.0)))
            self.texture_tor_strength = float(extra_state.get("texture_tor_strength", getattr(self, "texture_tor_strength", 1.0)))
            self.texture_shadow_confidence_enabled = bool(extra_state.get("texture_shadow_confidence_enabled", getattr(self, "texture_shadow_confidence_enabled", False)))
            self.texture_shadow_confidence_start_iter = int(extra_state.get("texture_shadow_confidence_start_iter", getattr(self, "texture_shadow_confidence_start_iter", 30_000)))
            self.texture_shadow_confidence_strength = float(extra_state.get("texture_shadow_confidence_strength", getattr(self, "texture_shadow_confidence_strength", 1.0)))
            self.texture_shadow_confidence_gamma = float(extra_state.get("texture_shadow_confidence_gamma", getattr(self, "texture_shadow_confidence_gamma", 1.0)))
            self.texture_shadow_confidence_zero_dc = bool(extra_state.get("texture_shadow_confidence_zero_dc", getattr(self, "texture_shadow_confidence_zero_dc", False)))
            self.texture_shadow_transport_mode = str(extra_state.get("texture_shadow_transport_mode", getattr(self, "texture_shadow_transport_mode", "additive"))).lower()
            self.texture_shadow_transport_eps = float(extra_state.get("texture_shadow_transport_eps", getattr(self, "texture_shadow_transport_eps", 1e-4)))
            self.texture_factor_surgery = str(extra_state.get("texture_factor_surgery", getattr(self, "texture_factor_surgery", "none"))).lower()
        ...
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        opt_dict = self._optimizer_state_for_current_groups_or_raise(
            opt_dict,
            allow_static_texture_pack=converted_static_textures,
            allow_new_texture_groups=bool(self.use_textures),
        )
    ...
    def _uses_texture_specular_tint(self):
        mode = str(getattr(self, "texture_effect_mode", "uvshadow_specular_lobe"))
        return mode in {"uvshadow_specular_tint", "uvshadow_ks_tint"}

    def _uses_texture_specular_lobe(self):
        mode = str(getattr(self, "texture_effect_mode", "uvshadow_specular_lobe"))
        return mode in {"uvshadow_specular_lobe", "uvshadow_roughness_residual"}

    def _texture_specular_channel_count(self):
        if self._uses_texture_specular_lobe():
            return 2
        return 3 if self._uses_texture_specular_tint() else 1

    @property
    def get_texture_specular_gain(self):
        if isinstance(self._tex_specular, torch.Tensor) and self._tex_specular.numel() > 0:
            raw_gain = self._tex_specular
            if self._uses_texture_specular_lobe() and raw_gain.shape[1] >= 1:
                raw_gain = raw_gain[:, 0:1] if raw_gain.ndim == 4 else raw_gain[:, 0:1]
            return torch.exp(2.0 * torch.tanh(raw_gain))
    ...
    @property
    def get_texture_specular_tint(self):
        if isinstance(self._tex_specular, torch.Tensor) and self._tex_specular.numel() > 0:
            return self.material_activation(self._tex_specular)
        device = self.get_xyz.device if isinstance(self.get_xyz, torch.Tensor) and self.get_xyz.numel() > 0 else "cuda"
        return torch.empty(0, device=device)
    ...
    def get_texture_specular_lobe_scale(self):
        if (
            isinstance(self._tex_specular, torch.Tensor)
            and self._tex_specular.numel() > 0
            and self._uses_texture_specular_lobe()
            and self._tex_specular.shape[1] >= 2
        ):
            raw_lobe = self._tex_specular[:, 1:2] if self._tex_specular.ndim == 4 else self._tex_specular[:, 1:2]
            return torch.exp(math.log(2.0) * torch.tanh(raw_lobe))
        if isinstance(self._tex_color, torch.Tensor) and self._tex_color.numel() > 0:
            if self._tex_color.ndim == 4:
                return torch.ones(
                    (self._tex_color.shape[0], 1, self._tex_color.shape[2], self._tex_color.shape[3]),
    ...
    def texture_architecture_state(self, include_optimizer=True):
        ...
        shadow_confidence_supported = mode in {
        ...
        spatial_shadow_stats = {
            "enabled": bool(getattr(self, "texture_shadow_spatial_resolution", False)),
            "hist": [],
            "total_texels": 0,
            "avg_texels_per_gaussian": 0.0,
            "min_resolution": 0,
            "max_resolution": 0,
            "estimated_float32_mb": 0.0,
        }
        if spatial_shadow_stats["enabled"] and isinstance(self.get_xyz, torch.Tensor):
            with torch.no_grad():
                texel_size = max(float(getattr(self, "texture_shadow_spatial_texel_size", 0.01)), 1e-8)
                min_res = max(1, int(getattr(self, "texture_shadow_spatial_min_resolution", 4)))
                max_res = max(min_res, int(getattr(self, "texture_shadow_spatial_max_resolution", 32)))
                sigma_factor = max(float(getattr(self, "texture_sigma_factor", 3.0)), 1e-6)
                scales = self.get_scaling.detach()
                if scales.ndim == 2 and scales.shape[0] > 0:
                    ...
                    if bool(getattr(self, "texture_shadow_spatial_power2", True)):
                        res_float = torch.pow(2.0, torch.ceil(torch.log2(res_float.clamp_min(1.0))))
                        res_float = res_float.clamp(min=min_res, max=max_res)
                    resolutions = res_float.to(torch.int64)
                    unique, counts = torch.unique(resolutions.cpu(), sorted=True, return_counts=True)
                    hist = [(int(r), int(c)) for r, c in zip(unique.tolist(), counts.tolist())]
                    total_texels = int((resolutions * resolutions).sum().item())
                    points = int(resolutions.numel())
                    spatial_shadow_stats.update(
                        {
                            "hist": hist,
                            "total_texels": total_texels,
                            "avg_texels_per_gaussian": float(total_texels) / max(1, points),
                            "min_resolution": int(resolutions.min().item()),
                            "max_resolution": int(resolutions.max().item()),
                            "estimated_float32_mb": float(total_texels * 4) / (1024.0 * 1024.0),
                        }
                    )
        ...
        state = {
            ...
            "route": {
                ...
                "per_uv_specular_residual": (not self._uses_texture_specular_tint()) and (not self._uses_texture_specular_lobe()) and ("specular" in mode or mode == "uv_specular_gain"),
                "per_uv_specular_lobe": bool(self._uses_texture_specular_lobe()),
                "per_uv_specular_tint": bool(self._uses_texture_specular_tint()),
                "per_uv_local_q": bool(self._uses_texture_local_q()),
                "per_uv_neural_phasefunc": mode == "per_uv",
                "deferred_texture_channels": 4 if mode in {"uvshadow_specular_residual", "uvshadow_specular_tint", "uvshadow_ks_tint", "uvshadow_specular_lobe", "uvshadow_roughness_residual"} else 7,
                "gaussian_other_effects": mode in {"uvshadow_specular_residual", "uvshadow_specular_tint", "uvshadow_ks_tint", "uvshadow_specular_lobe", "uvshadow_roughness_residual"},
            },
            "mbrdf": {
            ...
            "rtg": {
                ...
                "score_margin": float(getattr(self, "texture_rtg_score_margin", 1.0)),
                "gate_reference_resolution": float(getattr(self, "texture_rtg_gate_reference_resolution", 4.0)),
                ...
                "resize_mode": str(getattr(self, "texture_rtg_resize_mode", "activated_seeded_bilinear_exact")),
                "dry_run": bool(getattr(self, "texture_rtg_dry_run", False)),
                "stability_windows": int(getattr(self, "texture_rtg_stability_windows", 1)),
            ...
            "tor": {
                ...
                "gate_floor": float(getattr(self, "texture_tor_gate_floor", 0.05)),
                "gate_scale": float(getattr(self, "texture_tor_gate_scale", 1.0)),
                "strength": float(getattr(self, "texture_tor_strength", 1.0)),
            },
            "shadow_confidence": {
                "enabled": bool(getattr(self, "texture_shadow_confidence_enabled", False)),
                "mode_supported": bool(shadow_confidence_supported),
                "start_iter": int(getattr(self, "texture_shadow_confidence_start_iter", 30_000)),
                "strength": float(getattr(self, "texture_shadow_confidence_strength", 1.0)),
                "gamma": float(getattr(self, "texture_shadow_confidence_gamma", 1.0)),
                "zero_dc": bool(getattr(self, "texture_shadow_confidence_zero_dc", False)),
            },
            "shadow_transport": {
                "mode": str(getattr(self, "texture_shadow_transport_mode", "additive")),
                "eps": float(getattr(self, "texture_shadow_transport_eps", 1e-4)),
                "mode_supported": bool(shadow_confidence_supported),
            },
            "spatial_shadow": {
                "enabled": bool(getattr(self, "texture_shadow_spatial_resolution", False)),
                "texel_size": float(getattr(self, "texture_shadow_spatial_texel_size", 0.01)),
                "min_resolution": int(getattr(self, "texture_shadow_spatial_min_resolution", 4)),
                "max_resolution": int(getattr(self, "texture_shadow_spatial_max_resolution", 32)),
                "power2": bool(getattr(self, "texture_shadow_spatial_power2", True)),
                "hole_fill": str(getattr(self, "texture_shadow_hole_fill", "none")),
                "resolution_hist": spatial_shadow_stats["hist"],
                "total_shadow_texels": spatial_shadow_stats["total_texels"],
                "avg_shadow_texels_per_gaussian": spatial_shadow_stats["avg_texels_per_gaussian"],
                "actual_min_resolution": spatial_shadow_stats["min_resolution"],
                "actual_max_resolution": spatial_shadow_stats["max_resolution"],
                "estimated_float32_mb": spatial_shadow_stats["estimated_float32_mb"],
            },
            "factor_surgery": {
                "mode": str(getattr(self, "texture_factor_surgery", "none")),
                "seed": int(getattr(self, "texture_factor_surgery_seed", 0)),
            },
            "tensors": {
                ...
                "rtg_grad_accum": self._texture_tensor_summary(getattr(self, "_rtg_grad_accum", None)),
                "rtg_denom": self._texture_tensor_summary(getattr(self, "_rtg_denom", None)),
                "rtg_stability_count": self._texture_tensor_summary(getattr(self, "_rtg_stability_count", None)),
    ...
    def texture_architecture_log_string(self):
        ...
        if route.get("per_uv_specular_lobe", False):
            active.append("specular_lobe_uv")
        if route.get("per_uv_specular_tint", False):
            active.append("specular_tint_uv")
        ...
        return (
            ...
            f"cap_fraction={state['rtg']['refine_fraction']} min_score={state['rtg']['min_score']:.3e} "
            f"gate_ref={state['rtg']['gate_reference_resolution']:.2f} "
            f"gamma={state['rtg']['resolution_gamma']:.2f} "
            f"margin={state['rtg']['score_margin']:.2f} "
            f"stable_windows={state['rtg']['stability_windows']} "
            ...
            f"cnvr enabled={state['shadow_confidence']['enabled']} "
            f"supported={state['shadow_confidence']['mode_supported']} "
            f"start={state['shadow_confidence']['start_iter']} "
            f"strength={state['shadow_confidence']['strength']:.2f} "
            f"gamma={state['shadow_confidence']['gamma']:.2f} "
            f"zero_dc={state['shadow_confidence']['zero_dc']} | "
            f"shadow_transport mode={state['shadow_transport']['mode']} "
            f"eps={state['shadow_transport']['eps']:.1e} "
            f"supported={state['shadow_transport']['mode_supported']} | "
            f"spatial_shadow enabled={state['spatial_shadow']['enabled']} "
            f"texel={state['spatial_shadow']['texel_size']:.3g} "
            f"res={state['spatial_shadow']['min_resolution']}-{state['spatial_shadow']['max_resolution']} "
            f"power2={state['spatial_shadow']['power2']} "
            f"fill={state['spatial_shadow']['hole_fill']}"
        )

    def texture_rtg_probe_log_string(self):
        log = getattr(self, "_last_rtg_probe_log", None)
        if not log:
            return ""
        if log.get("skipped", False):
            return (
                f"target {int(log.get('target_iteration', log.get('iteration', 0)))}, "
                f"skipped {log.get('reason', 'unknown')}"
            )
        return (
            f"target {int(log.get('target_iteration', log.get('iteration', 0)))}, "
            f"window {log.get('probe_interval', 0)}/{log.get('refine_interval', 0)}, "
            f"pass {log.get('required_probe_windows', 0)}/{log.get('probe_windows', 0)}, "
            f"raw_window {log.get('raw_window_candidate_count', 0)}, "
            f"would_refine {log.get('would_refine_required_count', 0)} "
            f"(strict {log.get('would_refine_full_count', 0)}), "
            f"eligible {log.get('eligible_count', 0)}, "
            f"touched {log.get('window_touched_count', 0)}, "
            f"gate mean/max {log.get('gate_mean', 0.0):.3e}/{log.get('gate_max', 0.0):.3e}, "
            f"gate_ref {log.get('gate_reference_resolution', 0.0):.2f}, "
            f"gamma {log.get('resolution_gamma', 0.0):.2f}, "
            f"pass mean/p50/p90/p95/max "
            f"{log.get('probe_pass_mean', 0.0):.2f}/{log.get('probe_pass_p50', 0.0):.2f}/"
            f"{log.get('probe_pass_p90', 0.0):.2f}/{log.get('probe_pass_p95', 0.0):.2f}/"
            f"{log.get('probe_pass_max', 0.0):.2f}, "
            f"margin {log.get('score_margin', 1.0):.2f}"
        )
    ...
    def texture_rtg_log_string(self):
        ...
        if log.get("skipped", False):
            return (
                ...
                f"gamma {log.get('resolution_gamma', 0.0):.2f}, "
                f"margin {log.get('score_margin', 1.0):.2f}, "
                f"denom mean/p50/p95 {log.get('denom_mean', 0.0):.1f}/"
                f"{log.get('denom_p50', 0.0):.1f}/{log.get('denom_p95', 0.0):.1f}, "
                f"candidates {log.get('candidate_count', 0)}/{log.get('eligible_count', 0)} "
                f"(raw {log.get('raw_candidate_count', log.get('candidate_count', 0))}, "
                f"wait {log.get('stability_waiting_count', 0)}, "
                f"stable {log.get('stability_windows', 1)}w, "
                f"probe {log.get('required_probe_windows', 0)}/{log.get('probe_windows', 0)}), "
        ...
        return (
            ...
            f"score mean/p50/p90/p95/p99/max "
            f"{log['score_mean']:.3e}/{log.get('score_p50', 0.0):.3e}/"
            f"{log.get('score_p90', 0.0):.3e}/{log.get('score_p95', 0.0):.3e}/"
            f"{log.get('score_p99', 0.0):.3e}/{log['score_max']:.3e}, "
            ...
            f"(raw {log.get('raw_candidate_count', log['candidate_count'])}, "
            f"wait {log.get('stability_waiting_count', 0)}, "
            f"stable {log.get('stability_windows', 1)}w, "
            f"probe {log.get('required_probe_windows', 0)}/{log.get('probe_windows', 0)}), "
            f"cap {log.get('budget_count', 0)}, dry_run={log.get('dry_run', False)}, "
            f"resize {getattr(self, 'texture_rtg_resize_mode', 'activated_seeded_bilinear_exact')}, "
            f"res {{{hist}}}"
        )
    ...
    def _invalidate_dynamic_texture_layout_cache(self):
        self._dynamic_texture_layout_version = int(getattr(self, "_dynamic_texture_layout_version", 0)) + 1
        self._dynamic_texture_flat_ids_cache = None
    ...
    def _ensure_rtg_buffers(self):
        ...
        strict_dynamic = bool(getattr(self, "has_dynamic_textures", False))

        def ensure(name, dtype):
            value = getattr(self, name)
            if isinstance(value, torch.Tensor) and value.numel() == n_points:
                return
            if strict_dynamic and isinstance(value, torch.Tensor) and value.numel() > 0:
                raise RuntimeError(
                    f"RTG buffer '{name}' length {value.numel()} does not match point count {n_points}."
                )
            setattr(self, name, torch.zeros((n_points,), dtype=dtype, device=self.get_xyz.device))

        ensure("_rtg_score", torch.float32)
        ensure("_rtg_grad_accum", torch.float32)
        ensure("_rtg_denom", torch.float32)
        ensure("_rtg_stability_count", torch.int16)
    ...
    def _refresh_rtg_score_from_window(self):
        ...
        valid = self._rtg_denom > 0
        self._rtg_score[valid] = self._rtg_grad_accum[valid] / self._rtg_denom[valid].clamp_min(1.0)
        self._rtg_score.nan_to_num_(nan=0.0, posinf=0.0, neginf=0.0)
        return self._rtg_score

    def texture_rtg_due_target(self, iteration):
        if not bool(getattr(self, "texture_rtg_enabled", False)):
            return None
        rtg_from = int(getattr(self, "texture_rtg_refine_from_iter", 0))
        rtg_until = int(getattr(self, "texture_rtg_refine_until_iter", 0))
        interval = max(1, int(getattr(self, "texture_rtg_refine_interval", 1)))
        iteration = int(iteration)
        if iteration <= rtg_from or iteration > rtg_until:
            return None
        elapsed = iteration - rtg_from
        window = elapsed // interval
        if window <= 0:
            return None
        target = rtg_from + window * interval
        if target > rtg_until:
            return None
        if target <= int(getattr(self, "_last_rtg_refine_target_iter", 0)):
            return None
        return int(target)

    def _texture_rtg_probe_mode_enabled(self):
        probe_interval = int(getattr(self, "texture_rtg_probe_interval", 0))
        refine_interval = max(1, int(getattr(self, "texture_rtg_refine_interval", 1)))
        required = int(getattr(self, "texture_rtg_required_probe_windows", 0))
        return probe_interval > 0 and probe_interval < refine_interval and required > 0

    def _texture_rtg_probe_total_windows(self):
        probe_interval = max(1, int(getattr(self, "texture_rtg_probe_interval", 1)))
        refine_interval = max(1, int(getattr(self, "texture_rtg_refine_interval", 1)))
        return max(1, refine_interval // probe_interval)

    def _texture_rtg_required_probe_windows(self):
        total = self._texture_rtg_probe_total_windows()
        required = int(getattr(self, "texture_rtg_required_probe_windows", 0))
        if required <= 0:
            required = total
        return max(1, min(total, required))

    def texture_rtg_probe_target(self, iteration):
        if not bool(getattr(self, "texture_rtg_enabled", False)) or not self._texture_rtg_probe_mode_enabled():
            return None
        rtg_from = int(getattr(self, "texture_rtg_refine_from_iter", 0))
        rtg_until = int(getattr(self, "texture_rtg_refine_until_iter", 0))
        probe_interval = max(1, int(getattr(self, "texture_rtg_probe_interval", 1)))
        ...
        if iteration <= rtg_from or iteration > rtg_until:
            return None
        elapsed = iteration - rtg_from
        if elapsed <= 0 or elapsed % probe_interval != 0:
            return None
        target = rtg_from + (elapsed // probe_interval) * probe_interval
        if target <= int(getattr(self, "_last_rtg_probe_target_iter", 0)):
            return None
        return int(target)

    def _mark_texture_rtg_target_done(self, target_iteration):
        self._last_rtg_refine_target_iter = max(
            int(getattr(self, "_last_rtg_refine_target_iter", 0)),
            int(target_iteration),
        )

    def _mark_texture_rtg_probe_done(self, target_iteration):
        self._last_rtg_probe_target_iter = max(
            int(getattr(self, "_last_rtg_probe_target_iter", 0)),
            int(target_iteration),
        )
    ...
    def defer_texture_training(self):
        ...
        self._rtg_grad_accum = torch.empty(0, device=device)
        self._rtg_denom = torch.empty(0, device=device)
        self._rtg_stability_count = torch.empty(0, device=device, dtype=torch.int16)
        self._rtg_probe_pass_count = torch.empty(0, device=device, dtype=torch.int16)
        self._rtg_probe_window_count = torch.empty(0, device=device, dtype=torch.int16)
        self._rtg_probe_score_accum = torch.empty(0, device=device)
    ...
    def _texture_specular_initial_state(self, tex_color=None, texture_dims=None):
        tex = self._tex_color if tex_color is None else tex_color
        device = self.get_xyz.device if self.get_xyz.numel() > 0 else "cuda"
        if not isinstance(tex, torch.Tensor) or tex.numel() == 0:
            return torch.empty(0, device=device)

        channels = self._texture_specular_channel_count()
        if not self._uses_texture_specular_tint():
            shape = (
                (tex.shape[0], channels, tex.shape[2], tex.shape[3])
                if tex.ndim == 4
                else (tex.shape[0], channels)
            )
            return torch.zeros(shape, dtype=tex.dtype, device=tex.device)

        n_points = int(self.get_xyz.shape[0]) if isinstance(self.get_xyz, torch.Tensor) and self.get_xyz.numel() > 0 else 0
        if isinstance(self.ks, torch.Tensor) and self.ks.numel() == n_points * 3:
            base = self.ks.detach().to(device=tex.device, dtype=tex.dtype)
        else:
            base = torch.full((max(n_points, 1), 3), 0.5, dtype=tex.dtype, device=tex.device)

        if tex.ndim == 4:
            if base.shape[0] != tex.shape[0]:
                base = torch.full((tex.shape[0], 3), 0.5, dtype=tex.dtype, device=tex.device)
            return base[:, :, None, None].expand(-1, -1, tex.shape[2], tex.shape[3]).contiguous()

        if tex.ndim == 2:
            dims = self._texture_dims if texture_dims is None else texture_dims
            if isinstance(dims, torch.Tensor) and dims.numel() > 0 and dims.shape[0] == base.shape[0]:
                counts = (dims[:, 0].to(device=tex.device, dtype=torch.long) * dims[:, 1].to(device=tex.device, dtype=torch.long)).clamp_min(1)
                return torch.repeat_interleave(base, counts, dim=0)
            return torch.full((tex.shape[0], 3), 0.5, dtype=tex.dtype, device=tex.device)

        return torch.empty(0, device=tex.device, dtype=tex.dtype)

    def _initialize_texture_specular_state(self):
        if not (self.use_textures and self.use_mbrdf):
            return
        tex_specular = self._texture_specular_initial_state()
        if tex_specular.numel() == 0:
            self._tex_specular = tex_specular
            return
    ...
    def enable_texture_training(self, training_args):
        ...
        self.texture_rtg_refine_fraction = float(getattr(training_args, "texture_rtg_refine_fraction", 0.0))
        ...
        self.texture_rtg_min_score = float(getattr(training_args, "texture_rtg_min_score", 1e-6))
        self.texture_rtg_score_margin = float(getattr(training_args, "texture_rtg_score_margin", 1.0))
        self.texture_rtg_gate_reference_resolution = float(getattr(training_args, "texture_rtg_gate_reference_resolution", 4.0))
        ...
        self.texture_rtg_resize_mode = str(getattr(training_args, "texture_rtg_resize_mode", "activated_seeded_bilinear_exact")).lower()
        self.texture_rtg_dry_run = bool(getattr(training_args, "texture_rtg_dry_run", False))
        ...
        self.texture_tor_gate_scale = float(getattr(training_args, "texture_tor_gate_scale", 1.0))
        self.texture_tor_strength = float(getattr(training_args, "texture_tor_strength", 1.0))
        self.texture_specular_lr_scale = float(getattr(training_args, "texture_specular_lr_scale", 1.0))
        self.texture_normal_lr_scale = float(getattr(training_args, "texture_normal_lr_scale", 1.0))
    ...
    def _optimizer_state_for_current_groups_or_raise(
        self,
        opt_dict,
        allow_static_texture_pack=False,
        allow_new_texture_groups=False,
    ):
        texture_group_names = {"tex_color", "tex_alpha", "tex_specular", "tex_normal"}
        if self.optimizer is None or not isinstance(opt_dict, dict):
            raise RuntimeError("Cannot restore optimizer state: missing current optimizer or invalid checkpoint optimizer dict.")
        ...
        if not isinstance(saved_groups, list) or not isinstance(saved_state, dict):
            raise RuntimeError("Cannot restore optimizer state: malformed checkpoint optimizer state.")

        current_state_dict = self.optimizer.state_dict()
        current_groups = current_state_dict.get("param_groups", [])
        actual_param_by_state_id = {}
        for current_group, actual_group in zip(current_groups, self.optimizer.param_groups):
            for state_id, actual_param in zip(current_group.get("params", []), actual_group.get("params", [])):
                actual_param_by_state_id[state_id] = actual_param

        saved_by_name = {}
        for saved_group in saved_groups:
            if not isinstance(saved_group, dict):
                raise RuntimeError("Cannot restore optimizer state: malformed checkpoint param group.")
            name = saved_group.get("name")
            if name is None:
                raise RuntimeError("Cannot restore optimizer state: checkpoint param group is missing a name.")
            if name in saved_by_name:
                raise RuntimeError(f"Cannot restore optimizer state: duplicate checkpoint group name '{name}'.")
            saved_by_name[name] = saved_group
        ...
        extra_saved = set(saved_by_name) - current_names
        if extra_saved:
            raise RuntimeError(
                "Cannot restore optimizer state: checkpoint has groups that are absent in current optimizer: "
                f"{sorted(extra_saved)}."
            )
        ...
        for current_group in current_groups:
            ...
            if name is None:
                raise RuntimeError("Cannot restore optimizer state: current optimizer param group is missing a name.")
            current_params = current_group.get("params", [])
            saved_group = saved_by_name.get(name)
            if saved_group is None:
                if allow_new_texture_groups and name in texture_group_names:
                    restored_groups.append(dict(current_group))
                    continue
                raise RuntimeError(
                    f"Cannot restore optimizer state: current group '{name}' is missing in checkpoint."
                )

            saved_params = saved_group.get("params", [])
            if len(saved_params) != len(current_params):
                raise RuntimeError(
                    f"Cannot restore optimizer state for group '{name}': "
                    f"checkpoint has {len(saved_params)} params, current optimizer has {len(current_params)}."
                )
            restored_group = dict(saved_group)
            restored_group["params"] = list(current_params)
            ...
            for saved_param_id, current_param in zip(saved_params, current_params):
                ...
                actual_param = actual_param_by_state_id.get(current_param)
                if actual_param is None:
                    raise RuntimeError(
                        f"Cannot restore optimizer state for group '{name}': missing current parameter id {current_param}."
                    )
                if not isinstance(state, dict):
                    restored_state[current_param] = {}
                    continue
                current_state = {}
                for key, value in state.items():
                    if isinstance(value, torch.Tensor):
                        if key in {"exp_avg", "exp_avg_sq"} and value.shape != actual_param.shape:
                            if allow_static_texture_pack and name in texture_group_names and value.ndim == 4:
                                packed_value = self._pack_static_texture_tensor_for_dynamic(value)
                                if isinstance(packed_value, torch.Tensor) and packed_value.shape == actual_param.shape:
                                    value = packed_value
                                else:
                                    raise RuntimeError(
                                        f"Cannot restore optimizer state for group '{name}' key '{key}': "
                                        f"packed static texture state has shape {tuple(getattr(packed_value, 'shape', ()))}, "
                                        f"expected {tuple(actual_param.shape)}."
                                    )
                            else:
                                raise RuntimeError(
                                    f"Cannot restore optimizer state for group '{name}' key '{key}': "
                                    f"checkpoint tensor shape {tuple(value.shape)} does not match current shape "
                                    f"{tuple(actual_param.shape)}."
                                )
                        current_state[key] = value.to(device=actual_param.device, dtype=actual_param.dtype)
                    else:
                        current_state[key] = value
                restored_state[current_param] = current_state
        return {"state": restored_state, "param_groups": restored_groups}
    ...
    def _replace_dynamic_texture_tensors(
        ...
        if tex_specular is None and self.use_mbrdf:
            tex_specular = self._texture_specular_initial_state(tex_color, texture_dims)
    ...
    def _texture_tensor_for_optimizer_name(self, name):
        if name == "tex_color":
            return self._tex_color
        if name == "tex_alpha":
            return self._tex_alpha
        if name == "tex_specular":
            return self._tex_specular
        if name == "tex_normal":
            return self._tex_normal
        return None
        ...
        for key, value in local_state.items():
            ...
            if self._tex_normal.ndim == 4:
                ...
                if isinstance(dims, torch.Tensor) and dims.numel() > 0 and dims.shape[0] == value.shape[0]:
                    ...
                    raise RuntimeError(
                        "Cannot inherit tex_normal optimizer state: source local_q/rotation "
                        f"state shape {tuple(value.shape)} is incompatible with texture normal "
                        f"shape {tuple(self._tex_normal.shape)} and texture dims "
                        f"{tuple(getattr(self, '_texture_dims', torch.empty(0)).shape)}."
                    )
            else:
                raise RuntimeError(
                    "Cannot inherit tex_normal optimizer state: unsupported texture normal "
                    f"shape {tuple(self._tex_normal.shape)}."
                )
            if key == "exp_avg_sq":
        ...
        for key, value in kd_state.items():
            ...
            if self._tex_color.ndim == 4:
                ...
                if isinstance(dims, torch.Tensor) and dims.numel() > 0 and dims.shape[0] == value.shape[0]:
                    ...
                    raise RuntimeError(
                        "Cannot inherit tex_color optimizer state: source kd state shape "
                        f"{tuple(value.shape)} is incompatible with texture color shape "
                        f"{tuple(self._tex_color.shape)} and texture dims "
                        f"{tuple(getattr(self, '_texture_dims', torch.empty(0)).shape)}."
                    )
            else:
                raise RuntimeError(
                    "Cannot inherit tex_color optimizer state: unsupported texture color "
                    f"shape {tuple(self._tex_color.shape)}."
                )
            if key == "exp_avg_sq":
                expanded = expanded.clamp_min(0.0)
            out[key] = expanded
    ...
    def _gather_dynamic_optimizer_state_by_mask(self, name, point_mask):
        ...
        if not isinstance(state, dict):
            return None
        source_tensor = self._texture_tensor_for_optimizer_name(name)
        if not isinstance(source_tensor, torch.Tensor) or source_tensor.shape[0] != self._tex_color.shape[0]:
            raise RuntimeError(
                f"Cannot prune dynamic texture optimizer state for '{name}': "
                "texture tensor is missing or incompatible."
            )
        gathered_state = {}
        for key in ("exp_avg", "exp_avg_sq"):
            value = state.get(key)
            if value is None:
                value = torch.zeros_like(source_tensor)
            elif not isinstance(value, torch.Tensor) or value.shape != source_tensor.shape:
                raise RuntimeError(
                    f"Cannot prune dynamic texture optimizer state for '{name}': "
                    f"incompatible Adam state '{key}' shape "
                    f"{None if not isinstance(value, torch.Tensor) else tuple(value.shape)}, "
                    f"expected {tuple(source_tensor.shape)}."
                )
            gathered = self._gather_dynamic_tensor_by_mask(value, point_mask)
            if gathered is None:
                raise RuntimeError(
                    f"Cannot prune dynamic texture optimizer state for '{name}': "
                    f"missing or incompatible Adam state '{key}'."
                )
    ...
    def _append_dynamic_optimizer_state(self, name, selected_pts_mask, repeat_count=1):
        ...
        if not isinstance(source_tensor, torch.Tensor) or source_tensor.shape[0] != self._tex_color.shape[0]:
            raise RuntimeError(
                f"Cannot append dynamic texture optimizer state for '{name}': "
                "texture tensor is missing or incompatible."
            )
    ...
    def _resize_dynamic_optimizer_state(self, name, old_dims, new_resolutions):
        ...
        if not isinstance(source_tensor, torch.Tensor) or source_tensor.shape[0] != self._tex_color.shape[0]:
            raise RuntimeError(
                f"Cannot resize dynamic texture optimizer state for '{name}': "
                "texture tensor is missing or incompatible."
            )
            ...
            for res_value in torch.unique(old_resolutions[changed]).tolist():
                ...
                for target_res_tensor in torch.unique(new_resolutions[idx]).tolist():
                    ...
                    for chunk_start in range(0, int(target_idx.numel()), charts_per_chunk):
                        ...
                        up_value = self._rtg_upsample_grid(old_value, target_res)
                        if key == "exp_avg_sq":
    ...
    @staticmethod
    def _parent_split_upsample_grid(values, target_res):
        """Refine a texture chart by copying each parent texel into child texels.

        RTG refinement is a representation split, not a resampling filter.  A
        4x4 chart refined to 8x8 should initially carry the same per-cell values
        in each 2x2 child block, leaving optimization to learn the new detail.
        """
        if values.ndim != 4:
            raise ValueError("Expected texture grid values with shape [N, H, W, C].")
        old_h = int(values.shape[1])
        old_w = int(values.shape[2])
        if old_h <= 0 or old_w <= 0:
            raise ValueError("Cannot upsample an empty texture grid.")
        target_res = int(target_res)
        if target_res == old_h and target_res == old_w:
            return values.reshape(-1, values.shape[3])
        if target_res % old_h == 0 and target_res % old_w == 0:
            scale_y = target_res // old_h
            scale_x = target_res // old_w
            up = values.repeat_interleave(scale_y, dim=1).repeat_interleave(scale_x, dim=2)
        else:
            # Current RTG schedule uses exact doublings; this branch keeps
            # parent-copy behavior deterministic for unusual max resolutions.
            nchw = values.permute(0, 3, 1, 2).contiguous()
            up = F.interpolate(nchw, size=(target_res, target_res), mode="nearest")
            up = up.permute(0, 2, 3, 1).contiguous()
        return up.reshape(-1, values.shape[3])

    @staticmethod
    def _bilinear_exact_subdivide_grid(values, target_res):
        """Subdivide a bilinear texture function without changing its samples.

        The CUDA texture sampler uses align-corners coordinates:
        ``x = u * (width - 1)`` and ``y = v * (height - 1)``.  Under that
        convention, a render-preserving split from R to a denser grid is exact
        only when old grid knots remain knots in the new grid, e.g. R -> 2R-1.
        """
        if values.ndim != 4:
            raise ValueError("Expected texture grid values with shape [N, H, W, C].")
        nchw = values.permute(0, 3, 1, 2).contiguous()
        up = F.interpolate(nchw, size=(int(target_res), int(target_res)), mode="bilinear", align_corners=True)
        return up.permute(0, 2, 3, 1).contiguous().reshape(-1, values.shape[3])

    def _texture_rtg_resize_mode(self):
        mode = str(getattr(self, "texture_rtg_resize_mode", "activated_seeded_bilinear_exact")).lower()
        aliases = {
            "exact": "bilinear_exact",
            "exact_bilinear": "bilinear_exact",
            "bilinear": "bilinear_exact",
            "seeded": "seeded_bilinear_exact",
            "seeded_exact": "seeded_bilinear_exact",
            "seeded_bilinear": "seeded_bilinear_exact",
            "from1_exact": "seeded_bilinear_exact",
            "copy": "parent_copy",
            "parent": "parent_copy",
            "parent_split": "parent_copy",
            "parent_split_copy": "parent_copy",
            "activated": "activated_bilinear_exact",
            "activation": "activated_bilinear_exact",
            "activated_bilinear": "activated_bilinear_exact",
            "activation_bilinear": "activated_bilinear_exact",
            "activation_exact": "activated_bilinear_exact",
            "activated_seeded": "activated_seeded_bilinear_exact",
            "activated_seeded_exact": "activated_seeded_bilinear_exact",
            "activation_seeded": "activated_seeded_bilinear_exact",
            "activation_seeded_exact": "activated_seeded_bilinear_exact",
        }
        return aliases.get(mode, mode)

    def _rtg_upsample_grid(self, values, target_res):
        mode = self._texture_rtg_resize_mode()
        if mode in {"bilinear_exact", "activated_bilinear_exact"}:
            return self._bilinear_exact_subdivide_grid(values, target_res)
        if mode in {"seeded_bilinear_exact", "activated_seeded_bilinear_exact"}:
            if int(values.shape[1]) == 1 and int(values.shape[2]) == 1 and int(target_res) == 2:
                return self._parent_split_upsample_grid(values, target_res)
            return self._bilinear_exact_subdivide_grid(values, target_res)
        if mode == "parent_copy":
            return self._parent_split_upsample_grid(values, target_res)
        raise ValueError(f"Unknown texture_rtg_resize_mode: {mode}")

    def _texture_gain_activation(self, raw):
        return torch.exp(2.0 * torch.tanh(raw))

    def _texture_gain_inverse_activation(self, value):
        value = value.clamp(min=math.exp(-2.0) + 1e-6, max=math.exp(2.0) - 1e-6)
        return inverse_tanh(torch.log(value) / 2.0)

    def _texture_lobe_activation(self, raw):
        return torch.exp(math.log(2.0) * torch.tanh(raw))

    def _texture_lobe_inverse_activation(self, value):
        value = value.clamp(min=0.5 + 1e-6, max=2.0 - 1e-6)
        return inverse_tanh(torch.log(value) / math.log(2.0))

    def _texture_resize_activation(self, name, raw_values):
        if name == "tex_color":
            return self.material_activation(raw_values)
        if name == "tex_alpha":
            return torch.sigmoid(raw_values)
        if name == "tex_specular":
            if self._uses_texture_specular_tint():
                return self.material_activation(raw_values)
            if self._uses_texture_specular_lobe() and raw_values.shape[-1] >= 2:
                activated = raw_values.clone()
                activated[..., 0:1] = self._texture_gain_activation(raw_values[..., 0:1])
                activated[..., 1:2] = self._texture_lobe_activation(raw_values[..., 1:2])
                return activated
            return self._texture_gain_activation(raw_values)
        if name == "tex_normal":
            return F.normalize(raw_values, dim=-1, eps=1e-8)
        return raw_values

    def _texture_resize_inverse_activation(self, name, values):
        if name == "tex_color":
            return inverse_softplus(values)
        if name == "tex_alpha":
            return inverse_sigmoid(values.clamp(1e-6, 1.0 - 1e-6))
        if name == "tex_specular":
            if self._uses_texture_specular_tint():
                return inverse_softplus(values)
            if self._uses_texture_specular_lobe() and values.shape[-1] >= 2:
                raw = values.clone()
                raw[..., 0:1] = self._texture_gain_inverse_activation(values[..., 0:1])
                raw[..., 1:2] = self._texture_lobe_inverse_activation(values[..., 1:2])
                return raw
            return self._texture_gain_inverse_activation(values)
        if name == "tex_normal":
            return F.normalize(values, dim=-1, eps=1e-8)
        return values

    def _rtg_upsample_texture_grid(self, name, values, target_res):
        mode = self._texture_rtg_resize_mode()
        if mode not in {"activated_bilinear_exact", "activated_seeded_bilinear_exact"}:
            return self._rtg_upsample_grid(values, target_res)
        if mode == "activated_seeded_bilinear_exact" and int(values.shape[1]) == 1 and int(values.shape[2]) == 1 and int(target_res) == 2:
            return self._parent_split_upsample_grid(values, target_res)
    ...
    def _rtg_next_resolutions(self, old_resolutions):
        ...
        max_res = max(1, int(getattr(self, "texture_max_resolution", 1)))
        mode = self._texture_rtg_resize_mode()
        if mode in {"bilinear_exact", "activated_bilinear_exact"}:
            next_res = old_resolutions * 2 - 1
            return torch.where(next_res <= max_res, next_res, old_resolutions)
        if mode in {"seeded_bilinear_exact", "activated_seeded_bilinear_exact"}:
            next_res = torch.where(old_resolutions <= 1, torch.full_like(old_resolutions, 2), old_resolutions * 2 - 1)
            return torch.where(next_res <= max_res, next_res, old_resolutions)
        if mode == "parent_copy":
            return torch.clamp(old_resolutions * 2, max=max_res)
        raise ValueError(f"Unknown texture_rtg_resize_mode: {mode}")
    ...
    def _init_dynamic_textures_from_base_color(self, base_color):
        ...
        tex_specular = self._texture_specular_initial_state(tex_color, self._texture_dims)
        tex_normal = (
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        if self.use_textures:
            if self.texture_dynamic_resolution:
                ...
                self._tex_alpha = nn.Parameter(inverse_sigmoid(tex_alpha).requires_grad_(True))
                if self.use_mbrdf:
                    tex_specular = self._texture_specular_initial_state(tex_color)
                    self._tex_specular = nn.Parameter(tex_specular.requires_grad_(True))
    ...
    def save_appearance(self, path):
        ...
        if self.use_textures:
            ...
            payload["rtg_grad_accum"] = self._rtg_grad_accum.detach().cpu() if self._rtg_grad_accum.numel() > 0 else torch.empty(0)
            payload["rtg_denom"] = self._rtg_denom.detach().cpu() if self._rtg_denom.numel() > 0 else torch.empty(0)
            payload["rtg_stability_count"] = self._rtg_stability_count.detach().cpu() if self._rtg_stability_count.numel() > 0 else torch.empty(0, dtype=torch.int16)
    ...
    def load_appearance(self, path):
        ...
        self.texture_effect_mode = str(payload.get("texture_effect_mode", getattr(self, "texture_effect_mode", "uvshadow_specular_lobe")))
        ...
        self.texture_rtg_resize_mode = str(payload.get("texture_rtg_resize_mode", getattr(self, "texture_rtg_resize_mode", "activated_seeded_bilinear_exact"))).lower()
        self.texture_rtg_dry_run = bool(payload.get("texture_rtg_dry_run", getattr(self, "texture_rtg_dry_run", False)))
        self.texture_rtg_stability_windows = int(payload.get("texture_rtg_stability_windows", getattr(self, "texture_rtg_stability_windows", 1)))
        self.texture_rtg_probe_interval = int(payload.get("texture_rtg_probe_interval", getattr(self, "texture_rtg_probe_interval", 0)))
        ...
        self.texture_tor_gate_scale = float(payload.get("texture_tor_gate_scale", getattr(self, "texture_tor_gate_scale", 1.0)))
        self.texture_tor_strength = float(payload.get("texture_tor_strength", getattr(self, "texture_tor_strength", 1.0)))
        self.texture_shadow_confidence_enabled = bool(payload.get("texture_shadow_confidence_enabled", getattr(self, "texture_shadow_confidence_enabled", False)))
        self.texture_shadow_confidence_start_iter = int(payload.get("texture_shadow_confidence_start_iter", getattr(self, "texture_shadow_confidence_start_iter", 30_000)))
        self.texture_shadow_confidence_strength = float(payload.get("texture_shadow_confidence_strength", getattr(self, "texture_shadow_confidence_strength", 1.0)))
        self.texture_shadow_confidence_gamma = float(payload.get("texture_shadow_confidence_gamma", getattr(self, "texture_shadow_confidence_gamma", 1.0)))
        self.texture_shadow_confidence_zero_dc = bool(payload.get("texture_shadow_confidence_zero_dc", getattr(self, "texture_shadow_confidence_zero_dc", False)))
        self.texture_shadow_transport_mode = str(payload.get("texture_shadow_transport_mode", getattr(self, "texture_shadow_transport_mode", "additive"))).lower()
        self.texture_shadow_transport_eps = float(payload.get("texture_shadow_transport_eps", getattr(self, "texture_shadow_transport_eps", 1e-4)))
        self.texture_factor_surgery = str(payload.get("texture_factor_surgery", getattr(self, "texture_factor_surgery", "none"))).lower()
        ...
        if self.use_textures:
            if "tex_color" in payload:
                ...
                self._rtg_score = rtg_score.to(device="cuda", dtype=torch.float32) if isinstance(rtg_score, torch.Tensor) else torch.empty(0, device="cuda")
                rtg_grad_accum = payload.get("rtg_grad_accum", torch.empty(0))
                self._rtg_grad_accum = rtg_grad_accum.to(device="cuda", dtype=torch.float32) if isinstance(rtg_grad_accum, torch.Tensor) else torch.empty(0, device="cuda")
                rtg_denom = payload.get("rtg_denom", torch.empty(0))
                self._rtg_denom = rtg_denom.to(device="cuda", dtype=torch.float32) if isinstance(rtg_denom, torch.Tensor) else torch.empty(0, device="cuda")
                rtg_stability_count = payload.get("rtg_stability_count", torch.empty(0))
                self._rtg_stability_count = rtg_stability_count.to(device="cuda", dtype=torch.int16) if isinstance(rtg_stability_count, torch.Tensor) else torch.empty(0, device="cuda", dtype=torch.int16)
                rtg_probe_pass_count = payload.get("rtg_probe_pass_count", torch.empty(0))
                self._rtg_probe_pass_count = rtg_probe_pass_count.to(device="cuda", dtype=torch.int16) if isinstance(rtg_probe_pass_count, torch.Tensor) else torch.empty(0, device="cuda", dtype=torch.int16)
                rtg_probe_window_count = payload.get("rtg_probe_window_count", torch.empty(0))
                self._rtg_probe_window_count = rtg_probe_window_count.to(device="cuda", dtype=torch.int16) if isinstance(rtg_probe_window_count, torch.Tensor) else torch.empty(0, device="cuda", dtype=torch.int16)
    ...
    def _prune_dynamic_textures(self, valid_points_mask):
        ...
        texture_dims = self._build_texture_dims_from_resolutions(resolutions)
        expected = valid_points_mask.shape[0]
        for name in (
            "_rtg_score",
            "_rtg_grad_accum",
            "_rtg_denom",
            "_rtg_stability_count",
            ...
            if value.numel() != expected:
                raise RuntimeError(
                    f"Cannot prune dynamic textures: RTG buffer '{name}' length {value.numel()} "
                    f"does not match point count {expected}."
                )
        self._rtg_score = self._rtg_score[valid_points_mask]
        self._rtg_grad_accum = self._rtg_grad_accum[valid_points_mask]
        self._rtg_denom = self._rtg_denom[valid_points_mask]
        self._rtg_stability_count = self._rtg_stability_count[valid_points_mask]
    ...
    def _append_rtg_buffer_from_mask(self, name, selected_pts_mask, repeat_count=1):
        ...
        if not isinstance(value, torch.Tensor):
            return
        if value.numel() != selected_pts_mask.shape[0]:
            raise RuntimeError(
                f"Cannot append RTG buffer '{name}': buffer length {value.numel()} "
                f"does not match point count {selected_pts_mask.shape[0]}."
            )
        extension = torch.zeros(
            (int(selected_pts_mask.sum().item()) * repeat_count,),
            dtype=value.dtype,
            device=value.device,
        )
    ...
    def _append_dynamic_textures_from_mask(self, selected_pts_mask, repeat_count=1):
        ...
        self._append_rtg_buffer_from_mask("_rtg_score", selected_pts_mask, repeat_count)
        self._append_rtg_buffer_from_mask("_rtg_grad_accum", selected_pts_mask, repeat_count)
        self._append_rtg_buffer_from_mask("_rtg_denom", selected_pts_mask, repeat_count)
        self._append_rtg_buffer_from_mask("_rtg_stability_count", selected_pts_mask, repeat_count)
    ...
    def reset_texture_rtg_scores(self):
        self._ensure_rtg_buffers()
        self._rtg_score.zero_()
        self._rtg_grad_accum.zero_()
        self._rtg_denom.zero_()

    def reset_texture_rtg_probe_state(self):
        self._ensure_rtg_buffers()
        self._rtg_probe_pass_count.zero_()
        self._rtg_probe_window_count.zero_()
        self._rtg_probe_score_accum.zero_()

    def _texture_rtg_gate_tensors(self):
        old_resolutions = self._texture_dims[:, 0].to(torch.long)
        next_resolutions = self._rtg_next_resolutions(old_resolutions)
        ...
        base_gate = max(0.0, float(getattr(self, "texture_rtg_min_score", 1e-6)))
        score_margin = max(0.0, float(getattr(self, "texture_rtg_score_margin", 1.0)))
        gate_ref = max(1e-6, float(getattr(self, "texture_rtg_gate_reference_resolution", min_res)))
        resolution_scale = (old_resolutions.to(torch.float32) / gate_ref).clamp_min(1e-6).pow(gamma)
        score_gate = resolution_scale * base_gate * score_margin
        return old_resolutions, next_resolutions, eligible, resolution_scale, score_gate, base_gate, score_margin

    def update_texture_rtg_probe_window(self, iteration):
        self._last_rtg_probe_log = {}
        target_iteration = self.texture_rtg_probe_target(iteration)
        ...
        if not all(flags.values()):
            ...
            self._mark_texture_rtg_probe_done(target_iteration)
            self.reset_texture_rtg_scores()
            self.reset_texture_rtg_probe_state()
            return self.texture_rtg_probe_log_string()
        self._ensure_rtg_buffers()
        self._refresh_rtg_score_from_window()
        old_resolutions, _next_resolutions, eligible, _resolution_scale, score_gate, base_gate, score_margin = self._texture_rtg_gate_tensors()
        window_valid = self._rtg_denom > 0
        valid = torch.logical_and(eligible, window_valid)
        if base_gate > 0:
            valid = torch.logical_and(valid, self._rtg_score >= score_gate)
        total_windows = self._texture_rtg_probe_total_windows()
        required_windows = self._texture_rtg_required_probe_windows()
        pass_count = self._rtg_probe_pass_count.to(dtype=torch.int32)
        window_count = self._rtg_probe_window_count.to(dtype=torch.int32)
        pass_count = torch.where(
            valid,
            torch.clamp(pass_count + 1, max=total_windows),
            pass_count,
        )
        window_count = torch.where(
            eligible,
            torch.clamp(window_count + 1, max=total_windows),
            window_count,
        )
        self._rtg_probe_pass_count = pass_count.to(dtype=torch.int16)
        self._rtg_probe_window_count = window_count.to(dtype=torch.int16)
        self._rtg_probe_score_accum.add_(torch.where(window_valid, self._rtg_score, torch.zeros_like(self._rtg_score)))
        full_valid = torch.logical_and(
            eligible,
            torch.logical_and(self._rtg_probe_window_count >= total_windows, self._rtg_probe_pass_count >= total_windows),
        )
        required_valid = torch.logical_and(
            eligible,
            torch.logical_and(self._rtg_probe_window_count >= required_windows, self._rtg_probe_pass_count >= required_windows),
        )
        pass_values = self._rtg_probe_pass_count[eligible].detach().to(torch.float32)
        window_values = self._rtg_probe_window_count[eligible].detach().to(torch.float32)
        self._last_rtg_probe_log = {
            "iteration": int(iteration),
            "target_iteration": int(target_iteration),
            "probe_interval": int(getattr(self, "texture_rtg_probe_interval", 0)),
            "refine_interval": int(getattr(self, "texture_rtg_refine_interval", 0)),
            "probe_windows": int(total_windows),
            "required_probe_windows": int(required_windows),
            "raw_window_candidate_count": int(valid.sum().item()),
            "would_refine_required_count": int(required_valid.sum().item()),
            "would_refine_full_count": int(full_valid.sum().item()),
            "eligible_count": int(eligible.sum().item()),
            "window_touched_count": int(window_valid.sum().item()),
            "score_margin": float(score_margin),
            "gate_reference_resolution": float(getattr(self, "texture_rtg_gate_reference_resolution", 4.0)),
            "resolution_gamma": float(getattr(self, "texture_rtg_resolution_gamma", 1.0)),
            "gate_mean": float(score_gate[eligible].mean().item()) if bool(eligible.any().item()) else 0.0,
            "gate_max": float(score_gate[eligible].max().item()) if bool(eligible.any().item()) else 0.0,
            "resolution_hist": self._dynamic_texture_stats()["hist"],
        }
        self._last_rtg_probe_log.update(self._rtg_distribution_stats(pass_values, "probe_pass"))
        self._last_rtg_probe_log.update(self._rtg_distribution_stats(window_values, "probe_window"))
        self._mark_texture_rtg_probe_done(target_iteration)
        log = self.texture_rtg_probe_log_string()
        self.reset_texture_rtg_scores()
        return log

    def accumulate_texture_rtg(self, iteration=None, visibility_filter=None):
        ...
        if iteration is not None and int(iteration) <= int(self.texture_rtg_refine_from_iter):
            self.reset_texture_rtg_scores()
            if self._texture_rtg_probe_mode_enabled():
                self.reset_texture_rtg_probe_state()
            return
        if (
        ...
        with torch.no_grad():
            ...
            if isinstance(visibility_filter, torch.Tensor) and visibility_filter.numel() == n_points:
                touched = visibility_filter.to(device=score.device, dtype=torch.bool)
            else:
                touched = score > 0
            score = torch.where(touched, score, torch.zeros_like(score))
            self._rtg_grad_accum.add_(score)
            self._rtg_denom.add_(touched.to(dtype=self._rtg_denom.dtype))
            self._refresh_rtg_score_from_window()

    @staticmethod
    def _rtg_distribution_stats(values, prefix):
        ...
        q = torch.quantile(values, torch.tensor([0.5, 0.9, 0.95, 0.99], device=values.device))
        return {
            ...
            f"{prefix}_p99": float(q[3].item()),
        }

    def refine_textures_by_rtg(self, iteration):
        self._last_rtg_refine_log = {}
        target_iteration = self.texture_rtg_due_target(iteration)
        if target_iteration is None:
            return 0
        flags = {
            "use_textures": bool(self.use_textures),
        ...
        if not all(flags.values()):
            ...
            self._mark_texture_rtg_target_done(target_iteration)
            self.reset_texture_rtg_scores()
            return 0
        self._ensure_rtg_buffers()
        probe_mode = self._texture_rtg_probe_mode_enabled()
        if probe_mode:
            total_probe_windows = self._texture_rtg_probe_total_windows()
            required_probe_windows = self._texture_rtg_required_probe_windows()
            observed = self._rtg_probe_window_count > 0
            self._rtg_score.zero_()
            self._rtg_score[observed] = (
                self._rtg_probe_score_accum[observed]
                / self._rtg_probe_window_count[observed].to(dtype=torch.float32).clamp_min(1.0)
            )
            self._rtg_score.nan_to_num_(nan=0.0, posinf=0.0, neginf=0.0)
        else:
            total_probe_windows = 0
            required_probe_windows = 0
            self._refresh_rtg_score_from_window()
        old_resolutions, next_resolutions, eligible, resolution_scale, score_gate, base_gate, score_margin = self._texture_rtg_gate_tensors()
        ...
        if num_eligible == 0:
            ...
            return 0
        if probe_mode:
            full_window_valid = self._rtg_probe_window_count >= total_probe_windows
            window_valid = self._rtg_probe_window_count >= required_probe_windows
            raw_valid = torch.logical_and(eligible, torch.logical_and(window_valid, self._rtg_probe_pass_count > 0))
            valid = torch.logical_and(
                eligible,
                torch.logical_and(
                    self._rtg_probe_window_count >= required_probe_windows,
                    self._rtg_probe_pass_count >= required_probe_windows,
                ),
            )
            raw_candidate_count = int(raw_valid.sum().item())
            stability_windows = 1
        else:
            window_valid = self._rtg_denom > 0
            valid = torch.logical_and(eligible, window_valid)
            if base_gate > 0:
                valid = torch.logical_and(valid, self._rtg_score >= score_gate)
            raw_candidate_count = int(valid.sum().item())
            stability_windows = max(1, int(getattr(self, "texture_rtg_stability_windows", 1)))
            if stability_windows > 1:
                stability_count = self._rtg_stability_count.to(dtype=torch.int32)
                stability_count = torch.where(
                    valid,
                    torch.clamp(stability_count + 1, max=stability_windows),
                    torch.zeros_like(stability_count),
                )
                self._rtg_stability_count = stability_count.to(dtype=torch.int16)
                valid = torch.logical_and(valid, self._rtg_stability_count >= stability_windows)
        candidate_count = int(valid.sum().item())
        stability_waiting_count = max(0, raw_candidate_count - candidate_count)
        ...
        eligible_denom = (
            self._rtg_probe_window_count[eligible].detach().to(torch.float32)
            if probe_mode
            else self._rtg_denom[eligible].detach()
        )
        base_log = {
            "iteration": int(iteration),
            "target_iteration": int(target_iteration),
            ...
            "stability_waiting_count": int(stability_waiting_count),
            "stability_windows": int(stability_windows),
            "probe_mode": bool(probe_mode),
            "probe_interval": int(getattr(self, "texture_rtg_probe_interval", 0)),
            "probe_windows": int(total_probe_windows),
            "required_probe_windows": int(required_probe_windows),
            "would_refine_full_count": int(torch.logical_and(eligible, torch.logical_and(full_window_valid, self._rtg_probe_pass_count >= total_probe_windows)).sum().item()) if probe_mode else 0,
            "score_margin": float(score_margin),
            ...
            "score_gate_min": float(score_gate[eligible].min().item()) if eligible_scores.numel() > 0 else 0.0,
            "score_gate_max": float(score_gate[eligible].max().item()) if eligible_scores.numel() > 0 else 0.0,
            "gate_reference_resolution": float(getattr(self, "texture_rtg_gate_reference_resolution", 4.0)),
            "resolution_gamma": float(getattr(self, "texture_rtg_resolution_gamma", 1.0)),
        }
        base_log.update(self._rtg_distribution_stats(eligible_scores, "score"))
        base_log.update(self._rtg_distribution_stats(eligible_gates, "gate"))
        base_log.update(self._rtg_distribution_stats(eligible_denom, "denom"))
        if candidate_count == 0:
            self._last_rtg_refine_log = dict(base_log, **{
                "skipped": True,
        ...
        if 0.0 < fraction < 1.0:
            ...
            selected_idx = valid_idx[top_order]
        else:
            top_priority = priority[valid_idx]
        ...
        if top_priority.numel() == 0 or float(top_priority.max().item()) <= 0.0:
            ...
            log.update(self._rtg_distribution_stats(selected_scores, "selected_score"))
            log.update(self._rtg_distribution_stats(selected_gates, "selected_gate"))
            self._last_rtg_refine_log = log
            self._mark_texture_rtg_target_done(target_iteration)
            self.reset_texture_rtg_scores()
            if probe_mode:
                self.reset_texture_rtg_probe_state()
        ...
        selected_denom = (
            self._rtg_probe_window_count[selected].detach().to(torch.float32)
            if probe_mode
            else self._rtg_denom[selected].detach()
        )
        if bool(getattr(self, "texture_rtg_dry_run", False)):
            self._last_rtg_refine_log = dict(base_log, **{
                "skipped": True,
                "reason": "dry_run",
                "refined": int(selected.sum().item()),
            ...
            self._last_rtg_refine_log.update(self._rtg_distribution_stats(selected_scores, "selected_score"))
            self._last_rtg_refine_log.update(self._rtg_distribution_stats(selected_gates, "selected_gate"))
            self._last_rtg_refine_log.update(self._rtg_distribution_stats(selected_denom, "selected_denom"))
            self._mark_texture_rtg_target_done(target_iteration)
            self.reset_texture_rtg_scores()
            if probe_mode:
                self.reset_texture_rtg_probe_state()
        ...
        if refined > 0:
            if self._rtg_stability_count.numel() == selected.numel():
                self._rtg_stability_count[selected] = 0
                    ...
                    for key in ("exp_avg", "exp_avg_sq"):
                        ...
                        if isinstance(value, torch.Tensor):
                            if value.shape != tensor.shape:
                                raise RuntimeError(
                                    f"Cannot replace optimizer tensor '{name}' key '{key}': "
                                    f"state shape {tuple(value.shape)} does not match new tensor "
                                    f"shape {tuple(tensor.shape)}."
                                )
                            stored_state[key] = value.to(device=tensor.device, dtype=tensor.dtype)
                        elif key in stored_state:
                            if stored_state[key].shape == tensor.shape:
                                stored_state[key] = torch.zeros_like(tensor)
                            else:
                                raise RuntimeError(
                                    f"Cannot replace optimizer tensor '{name}': missing resized "
                                    f"Adam state '{key}' for existing optimizer state."
                                )
                        else:
                            stored_state[key] = torch.zeros_like(tensor)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            if len(group["params"]) != 1:
                continue
            if self.has_dynamic_textures and group["name"] in {
                "tex_color",
                "tex_alpha",
                "tex_specular",
                "tex_normal",
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                if not isinstance(stored_state, dict):
                    raise RuntimeError(f"Optimizer state for {group['name']} is not a dict.")
                current_param = group["params"][0]
                extension_state = state_extensions.get(group["name"], {})
                for key in ("exp_avg", "exp_avg_sq"):
                    base_state = stored_state.get(key)
                    if base_state is None:
                        base_state = torch.zeros_like(current_param)
                    elif not isinstance(base_state, torch.Tensor):
                        raise RuntimeError(f"Optimizer state {key} for {group['name']} is not a tensor.")
                    else:
                        base_state = base_state.to(device=current_param.device, dtype=current_param.dtype)
                        if base_state.shape != current_param.shape:
                            raise RuntimeError(
                                f"Optimizer state {key} for {group['name']} has shape "
                                f"{tuple(base_state.shape)}, expected {tuple(current_param.shape)}."
                            )
                    ...
                    stored_state[key] = torch.cat((base_state, inherited), dim=0)
                del self.optimizer.state[group["params"][0]]
    ...
    def densify_and_prune(
        ...
        extent,
        max_screen_size,
        bigsize_threshold=0.1,
        crop_extent=None,
        clone_grad_threshold=None,
    ):
        num_points_start = int(self.get_xyz.shape[0])
        clone_grad_threshold = float(max_grad if clone_grad_threshold is None else clone_grad_threshold)
        ...
        split_grad_mask = grad_values >= max_grad
        clone_grad_mask = grad_values >= clone_grad_threshold
        ...
        stats = {
            ...
            "densify_split_grad_threshold": float(max_grad),
            "densify_clone_grad_threshold": float(clone_grad_threshold),
            ...
            "densify_grad_candidate_count": int(split_grad_mask.sum().item()),
            "densify_clone_grad_candidate_count": int(clone_grad_mask.sum().item()),
            "densify_split_grad_candidate_count": int(split_grad_mask.sum().item()),
            "densify_clone_scale_count": int(clone_scale_mask.sum().item()),
            "densify_split_scale_count": int(split_scale_mask.sum().item()),
            "densify_clone_candidate_count": int(torch.logical_and(clone_grad_mask, clone_scale_mask).sum().item()),
            "densify_split_candidate_count": int(torch.logical_and(split_grad_mask, split_scale_mask).sum().item()),
        }
        ...
        num_clone = self.densify_and_clone(grads, clone_grad_threshold, extent)
