...
OUT_ROOT="${OUT_ROOT:-/ssdwork/liuhaohan/outputs/experiments/20260713_1545_eval/benchmark}"
...
benchmark_model() {
  ...
  local model="$3"
  local required
  for required in cfg_args chkpnt100000.pth point_cloud/iteration_100000/point_cloud.ply point_cloud/iteration_100000/transforms_test.json; do
    [[ -f "${model}/${required}" ]] || {
  ...
  case "${scene}" in
    LightStage/*|Synthetic/*) args+=(--hdr --gamma) ;;
  esac

  local log="${OUT_ROOT}/logs/${scene//\//_}_${method}.log"
  ...
  grep -q '^BENCHMARK_JSON ' "${log}" || {
    echo "No BENCHMARK_JSON record in ${log}" >&2
    exit 2
  }
  "${PYTHON}" - "${MANIFEST}" "${scene}" "${method}" "${model}" "${log}" <<'PY'
import json
import sys

manifest, scene, method, model, log = sys.argv[1:]
...
for scene in "${SCENE_ARRAY[@]}"; do
  ...
  rtd_model="$(find_rtd_model "${scene}")" || {
    echo "No completed RTD checkpoint found for ${scene}" >&2
    exit 2
  }
  benchmark_model "${scene}" "MicroTexGS-fixed" "${BASELINE_ROOT}/${scene}"
  benchmark_model "${scene}" "MicroTexGS-RTD" "${rtd_model}"
done

"${PYTHON}" "${SCRIPT_DIR}/summarize_benchmark.py" \
  --manifest "${MANIFEST}" \
