...
def ExrtoTorch(cv2_image, resolution):
...
def my_print(print_func):
    ...
    def wrap(*args, **kwargs):
        ...
        while call[2] in ('log', 'show'):
            i -= 1
            call = traceback.extract_stack()[i]
        print_func(f'\033[36m[{datetime.now().strftime("%H:%M:%S")} {os.path.basename(call[0])}:{call[1]}]\033[0m ', end='')
...
'''
    Gradient Scaling from EGSR 2023 paper:
