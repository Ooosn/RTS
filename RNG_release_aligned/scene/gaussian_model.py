...
class GaussianModel:
    ...
    def reset_features(self):
        dc_shape = self._features_dc.shape
        rest_shape = self._features_rest.shape
        ## randomize the features
        self._features_dc = nn.Parameter(torch.randn_like(self._features_dc))
        self._features_rest = nn.Parameter(torch.randn_like(self._features_rest))
    ...
    def crop_pc(self, extent):
        self.crop_boundary = extent
        if extent == 0:
            return
        
        mask = torch.logical_and(torch.abs(self.get_xyz) < extent, torch.abs(self.get_xyz) > -extent)
        mask = torch.all(mask, dim=1)
        num_cropped = (~mask).sum()
        print(f'Cropped {num_cropped} points.')
        if num_cropped == 0:
            return
