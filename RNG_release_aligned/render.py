...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, color_mlp = None, depth_mlp=None, dump_samples=False, dump_poses=False, relight_envmap=None, relight_spp=128, relight_intensity_scaling=0.0, compute_metrics=True, save_gt=True, hdr=False):
    if relight_envmap:
        raise NotImplementedError
        name += '_relight_' + os.path.basename(relight_envmap)
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders")
    makedirs(render_path, exist_ok=True)
    if save_gt and not relight_envmap:
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if pipeline.output_feature:
            ...
            rendering_for_output = _display_space(rendering, hdr)
            if pipeline.output_exr:
                exr.write(rendering.detach().cpu().numpy().transpose(1, 2, 0), os.path.join(render_path, '{0:05d}'.format(idx) + ".exr"))
            else:
                torchvision.utils.save_image(rendering_for_output, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
            if compute_metrics and save_gt and not relight_envmap:
                rendering_eval = torch.clamp(rendering_for_output, 0.0, 1.0)
                gt_eval = torch.clamp(gt, 0.0, 1.0)
                rendering_batch = rendering_eval.unsqueeze(0)
                gt_batch = gt_eval.unsqueeze(0)
                psnr_sum += psnr(rendering_batch, gt_batch).mean().double().item()
                ssim_sum += ssim(rendering_batch, gt_batch).mean().double().item()
                lpips_sum += get_lpips_vgg(rendering_eval.device).forward(
                    rendering_batch, gt_batch
                ).squeeze().item()
                metric_count += 1
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, render_videos=False, bitrate='10M', render_light=False, render_view=False, novel_view=None, collocated=False, frame_idx=0, frames=300, y_axis=False, crop_pc=0.0, dump_samples=False, dump_poses=False, relight_envmap=None, relight_spp=128, relight_intensity_scaling=0.0, render_valid=False, compute_metrics=True):
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, color_mlp=color_mlp, depth_mlp=depth_mlp, dump_samples=dump_samples, dump_poses=dump_poses, relight_envmap=relight_envmap, relight_spp=relight_spp, relight_intensity_scaling=relight_intensity_scaling, compute_metrics=compute_metrics, hdr=dataset.hdr)

        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, color_mlp=color_mlp, depth_mlp=depth_mlp, dump_samples=dump_samples, dump_poses=dump_poses, relight_envmap=relight_envmap, relight_spp=relight_spp, relight_intensity_scaling=relight_intensity_scaling, compute_metrics=compute_metrics, hdr=dataset.hdr)
        if render_valid:
             valid_cameras = scene.getValidCameras()
             if len(valid_cameras) == 0:
                 print("No valid cameras found.")
             else:
                 render_set(dataset.model_path, "valid", scene.loaded_iter, valid_cameras, gaussians, pipeline, background, color_mlp=color_mlp, depth_mlp=depth_mlp, dump_samples=dump_samples, dump_poses=dump_poses, relight_envmap=relight_envmap, relight_spp=relight_spp, relight_intensity_scaling=relight_intensity_scaling, compute_metrics=False, save_gt=False, hdr=dataset.hdr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_internal_metrics", action="store_true")
    ...
    args.hdr = args.hdr if hasattr(args, 'hdr') else False
    ...
    render_sets(
        ...
        render_valid=args.valid,
        compute_metrics=not args.skip_internal_metrics,
    )
