...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier = 1.0, override_color = None, color_mlp = None, depth_mlp = None, iteration = 0, dump_samples = False, relight_envmap = None):
    ...
    if getattr(viewpoint_camera, "cam_pose_adj", None) is not None and viewpoint_camera.cam_pose_adj.requires_grad:
        conic_grad_holder = torch.zeros(
            (pc.get_xyz.shape[0], 2, 2),
    ...
    rendered_image, radii, rendered_depth, rendered_alpha = rasterizer(
        ...
        conic_grad_holder = conic_grad_holder)
    ...
    ########################################################
    if hasattr(viewpoint_camera, "ensure_rays"):
        viewpoint_camera.ensure_rays()
